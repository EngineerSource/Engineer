...
if not DevAbs:get(Server.."IdEngineer") then 
io.write('\27[1;35m\nالان ارسل ايدي المطور الاساسي ↫ ⤈\n\27[0;33;49m') 
local DevId = io.read():gsub(' ','') 
if tostring(DevId):match('%d+') then 
io.write('\27[1;36mتم حفظ ايدي المطور الاساسي\n27[0;39;49m') 
DevAbs:set(Server.."IdEngineer",DevId) 
else 
print('\27[1;31m┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉\nلم يتم حفظ ايدي المطور الاساسي ارسله مره اخرى\n┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉') 
end 
os.execute('lua Engineer.lua') 
end 
if not DevAbs:get(Server.."TokenEngineer") then 
...
DevAbs:set(Server.."TokenEngineer",TokenBot) 
end  
else 
...
Config = {
DevId = DevAbs:get(Server.."IdEngineer"),
TokenBot = DevAbs:get(Server.."TokenEngineer"),
Engineer = DevAbs:get(Server.."TokenEngineer"):match("(%d+)"),
SudoIds = {DevAbs:get(Server.."IdEngineer")},
}
Create(Config, "./config.lua") 
file = io.open("Engineer.sh", "w")  
file:write([[
#!/usr/bin/env bash
cd $HOME/Engineer
token="]]..DevAbs:get(Server.."TokenEngineer")..[["
while(true) do
rm -fr ../.telegram-cli
if [ ! -f ./tg ]; then
echo "┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉ ≈ ┉"
...
local Load_Engineer = function() 
local f = io.open("./config.lua", "r") 
if not f then 
AutoSet() 
else 
f:close() 
DevAbs:del(Server.."IdEngineer");DevAbs:del(Server.."TokenEngineer")
end 
...
local Status = DevAbs:sismember(Engineer..'Abs:AbsSudo:',msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Engineer..'Abs:SecondSudo:',msg.sender_user_id_) 
...
function Bot(msg) 
local var = false  
if msg.sender_user_id_ == tonumber(Engineer) then  
var = true  
end  
return var  
end 
...
local Status = DevAbs:sismember(Engineer..'Abs:SudoBot:',msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Engineer..'Abs:AbsConstructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Engineer..'Abs:BasicConstructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Engineer..'Abs:Constructor:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Engineer..'Abs:Managers:'..msg.chat_id_,msg.sender_user_id_) 
...
----------  Admin  ---------
function Admin(msg) 
local Status = DevAbs:sismember(Engineer..'Abs:Admins:'..msg.chat_id_,msg.sender_user_id_) 
...
---------Vip Member---------
function VipMem(msg) 
local Status = DevAbs:sismember(Engineer..'Abs:VipMem:'..msg.chat_id_,msg.sender_user_id_) 
...
local Status = DevAbs:sismember(Engineer..'Abs:Cleaner:'..msg.chat_id_,msg.sender_user_id_) 
if Status or SudoBot(msg) or AbsConstructor(msg) or AbsSudo(msg) or Sudo(msg) or SecondSudo(msg) or Bot(msg) then  
return true  
else  
return false  
...
local Status = DevAbs:sismember(Engineer..'Abs:CleanerNum:'..msg.chat_id_,msg.sender_user_id_) 
...
if DevAbs:sismember(Engineer..'Abs:Ban:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(Engineer..'Abs:BanAll:', user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(Engineer..'Abs:Muted:'..chat_id, user_id) then
var = true
else
var = false
end
...
if DevAbs:sismember(Engineer..'Abs:MuteAll:', user_id) then
var = true
else
var = false
end
...
--     Source Engineer     --
function EngineerFiles(msg)
for v in io.popen('ls Files'):lines() do
if v:match(".lua$") then
plugin = dofile("Files/"..v)
if plugin.Engineer and msg then
FilesText = plugin.Engineer(msg)
end
end
end
...
if File_Name:lower():match('(%d+)') ~= Engineer:lower() then 
...
local info_file = io.open('./'..Engineer..'.json', "r"):read('*a')
local JsonInfo = JSON.decode(info_file)
vardump(JsonInfo)
DevAbs:set(Engineer.."Abs:NameBot",JsonInfo.BotName) 
for IdGps,v in pairs(JsonInfo.GroupsList) do
DevAbs:sadd(Engineer.."Abs:Groups",IdGps) 
DevAbs:set(Engineer.."Abs:Lock:Bots"..IdGps,"del") DevAbs:hset(Engineer.."Abs:Spam:Group:User"..IdGps ,"Spam:User","keed") 
...
DevAbs:set(Engineer..Lock..IdGps,true)
end
if v.AbsConstructors then
for k,IdAbsConstructors in pairs(v.AbsConstructors) do
DevAbs:sadd(Engineer..'Abs:AbsConstructor:'..IdGps,IdAbsConstructors)  
print('تم رفع منشئين المجموعات')
end
end
if v.BasicConstructors then
for k,IdBasicConstructors in pairs(v.BasicConstructors) do
DevAbs:sadd(Engineer..'Abs:BasicConstructor:'..IdGps,IdBasicConstructors)  
print('تم رفع ( '..k..' ) منشئين اساسيين')
end
end
if v.Constructors then
for k,IdConstructors in pairs(v.Constructors) do
DevAbs:sadd(Engineer..'Abs:Constructor:'..IdGps,IdConstructors)  
print('تم رفع ( '..k..' ) منشئين')
end
end
if v.Managers then
for k,IdManagers in pairs(v.Managers) do
DevAbs:sadd(Engineer..'Abs:Managers:'..IdGps,IdManagers)  
...
vardump(IdAdmins)
DevAbs:sadd(Engineer..'Abs:Admins:'..IdGps,IdAdmins)  
print('تم رفع ( '..k..' ) ادمنيه')
end
end
if v.Vips then
for k,IdVips in pairs(v.Vips) do
DevAbs:sadd(Engineer..'Abs:VipMem:'..IdGps,IdVips)  
print('تم رفع ( '..k..' ) مميزين')
end
end
if v.LinkGroups then
if v.LinkGroups ~= "" then
DevAbs:set(Engineer.."Abs:Groups:Links"..IdGps,v.LinkGroups)   
print('( تم وضع روابط المجموعات )')
end
end
if v.Welcomes then
if v.Welcomes ~= "" then
DevAbs:set(Engineer.."Abs:Groups:Welcomes"..IdGps,v.Welcomes)   
...
var = true  
elseif tonumber(user_id) == tonumber(Engineer) then  
var = true
elseif DevAbs:sismember(Engineer..'Abs:AbsSudo:', user_id) then
var = true
elseif DevAbs:sismember(Engineer..'Abs:SecondSudo:', user_id) then
var = true  
elseif DevAbs:sismember(Engineer..'Abs:SudoBot:', user_id) then
var = true 
elseif DevAbs:sismember(Engineer..'Abs:AbsConstructor:'..chat_id, user_id) then
var = true
elseif DevAbs:sismember(Engineer..'Abs:BasicConstructor:'..chat_id, user_id) then
var = true
elseif DevAbs:sismember(Engineer..'Abs:Constructor:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(Engineer..'Abs:Managers:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(Engineer..'Abs:Admins:'..chat_id, user_id) then
var = true  
elseif DevAbs:sismember(Engineer..'Abs:VipMem:'..chat_id, user_id) then  
var = true 
else  
var = false
end  
...
local Abs = (Engineer..'Abs:Filters:'..msg.chat_id_)
if Abs then
...
--     Source Engineer     --
function Absmoned(chat_id, user_id, msg_id, text, offset, length) local tt = DevAbs:get(Engineer..'endmsg') or '' tdcli_function ({ ID = "SendMessage", chat_id_ = chat_id, reply_to_message_id_ = msg_id, disable_notification_ = 0, from_background_ = 1, reply_markup_ = nil, input_message_content_ = { ID = "InputMessageText", text_ = text..'\n\n'..tt, disable_web_page_preview_ = 1, clear_draft_ = 0, entities_ = {[0]={ ID="MessageEntityMentionName", offset_=offset, length_=length, user_id_=user_id }, }, }, }, dl_cb, nil) end
--     Source Engineer     --
function ChCheck(msg)
local var = true 
if DevAbs:get(Engineer.."Abs:ChId") then
local url , res = https.request('https://api.telegram.org/bot'..TokenBot..'/getchatmember?chat_id='..DevAbs:get(Engineer..'Abs:ChId')..'&user_id='..msg.sender_user_id_)
local data = json:decode(url)
if res ~= 200 or data.result.status == "left" or data.result.status == "kicked" then
var = false 
if DevAbs:get(Engineer..'Abs:ChText') then
local ChText = DevAbs:get(Engineer..'Abs:ChText')
send(msg.chat_id_,msg.id_,'['..ChText..']')
else
local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChat?chat_id='..DevAbs:get(Engineer.."Abs:ChId"))
local GetInfo = JSON.decode(Check)
...
if DataText == '/delyes' and DevAbs:get(Engineer..'yes'..data.sender_user_id_) == 'delyes' then
DevAbs:del(Engineer..'yes'..data.sender_user_id_, 'delyes')
DevAbs:del(Engineer..'no'..data.sender_user_id_, 'delno')
...
if DataText == '/delno' and DevAbs:get(Engineer..'no'..data.sender_user_id_) == 'delno' then
DevAbs:del(Engineer..'yes'..data.sender_user_id_, 'delyes')
DevAbs:del(Engineer..'no'..data.sender_user_id_, 'delno')
EditMsg(Chat_Id2, Msg_Id2, "᥀︙تم الغاء امر اطردني") 
end
--     Source Engineer     --
if DataText == '/yesdel' and DevAbs:get(Engineer..'yesdel'..data.sender_user_id_) == 'delyes' then
DevAbs:del(Engineer..'yesdel'..data.sender_user_id_, 'delyes')
DevAbs:del(Engineer..'nodel'..data.sender_user_id_, 'delno')
if DevAbs:sismember(Engineer..'Abs:Constructor:'..data.chat_id_, data.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(Engineer..'Abs:Managers:'..data.chat_id_, data.sender_user_id_) then
Managers = 'المدراء • ' else Managers = '' end
if DevAbs:sismember(Engineer..'Abs:Admins:'..data.chat_id_, data.sender_user_id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevAbs:sismember(Engineer..'Abs:VipMem:'..data.chat_id_, data.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(Engineer..'Abs:Cleaner:'..data.chat_id_, data.sender_user_id_) then
cleaner = 'المنظفين • ' else cleaner = '' end
if DevAbs:sismember(Engineer..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
donky = 'المطايه • ' else donky = '' end
if DevAbs:sismember(Engineer..'Abs:Constructor:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(Engineer..'Abs:Managers:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(Engineer..'Abs:Admins:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(Engineer..'Abs:VipMem:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(Engineer..'Abs:Cleaner:'..data.chat_id_, data.sender_user_id_) or DevAbs:sismember(Engineer..'User:Donky:'..data.chat_id_, data.sender_user_id_) then
DevAbs:srem(Engineer..'Abs:Constructor:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(Engineer..'Abs:Managers:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(Engineer..'Abs:Admins:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(Engineer..'Abs:VipMem:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(Engineer..'Abs:Cleaner:'..data.chat_id_,data.sender_user_id_)
DevAbs:srem(Engineer..'User:Donky:'..data.chat_id_,data.sender_user_id_)
...
if DevAbs:get(Engineer.."Abs:NewDev"..data.sender_user_id_) then
if DataText == '/setno' then
EditMsg(Chat_Id2, Msg_Id2, "᥀︙تم الغاء امر تغير المطور الاساسي") 
DevAbs:del(Engineer.."Abs:NewDev"..data.sender_user_id_)
return false
end
if DataText == '/setyes' then
local NewDev = DevAbs:get(Engineer.."Abs:NewDev"..data.sender_user_id_)
...
if DataText == '/nodel' and DevAbs:get(Engineer..'nodel'..data.sender_user_id_) == 'delno' then
DevAbs:del(Engineer..'yesdel'..data.sender_user_id_, 'delyes')
DevAbs:del(Engineer..'nodel'..data.sender_user_id_, 'delno')
EditMsg(Chat_Id2, Msg_Id2, "᥀︙تم الغاء امر نزلني") 
end
if DataText == '/YesRolet' and DevAbs:get(Engineer.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_) then
local List = DevAbs:smembers(Engineer..'Abs:ListRolet'..data.chat_id_) 
local UserName = List[math.random(#List)]
tdcli_function ({ID="SearchPublicChat",username_ = UserName},function(arg,dp) 
DevAbs:incrby(Engineer..'Abs:GamesNumber'..data.chat_id_..dp.id_, 5) 
end,nil) 
DevAbs:del(Engineer..'Abs:ListRolet'..data.chat_id_) 
DevAbs:del(Engineer.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_)
EditMsg(Chat_Id2, Msg_Id2, "᥀︙*صاحب الحظ* ↫ ["..UserName.."]\n᥀︙*مبروك لقد ربحت وحصلت على 5 نقاط يمكنك استبدالها بالرسائل*")
end
if DataText == '/NoRolet' then
DevAbs:del(Engineer..'Abs:ListRolet'..data.chat_id_) 
DevAbs:del(Engineer.."Abs:NumRolet"..data.chat_id_..data.sender_user_id_) 
DevAbs:del(Engineer.."Abs:WittingStartRolet"..data.chat_id_..data.sender_user_id_)
...
if DevAbs:sismember(Engineer..'Abs:Tkeed:'..Chat_Id2, data.sender_user_id_) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..Chat_Id2.."&user_id="..data.sender_user_id_.."&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")
DevAbs:srem(Engineer..'Abs:Tkeed:'..Chat_Id2, data.sender_user_id_)
...
DevAbs:del(Engineer..'Abs:Text:GpTexts'..Abs..data.chat_id_)
DevAbs:srem(Engineer..'Abs:Manager:GpRedod'..data.chat_id_,Abs)
end
if DataText and DataText:match('/EndRedod:'..tonumber(data.sender_user_id_)..'(.*)') then
local Abs = DataText:match('/EndRedod:'..tonumber(data.sender_user_id_)..'(.*)')
local List = DevAbs:smembers(Engineer..'Abs:Text:GpTexts'..Abs..data.chat_id_)
if DevAbs:get(Engineer..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_) then
EditMsg(Chat_Id2, Msg_Id2, "᥀︙تم انهاء وحفظ ↫ "..#List.." من الردود المتعدده للامر ↫ "..Abs) 
DevAbs:del(Engineer..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Add:GpRedod'..data.sender_user_id_..data.chat_id_)
DevAbs:del(Engineer..'Abs:Text:GpTexts'..Abs..data.chat_id_)
DevAbs:del(Engineer..'Abs:Add:GpTexts'..data.sender_user_id_..data.chat_id_)
DevAbs:srem(Engineer..'Abs:Manager:GpRedod'..data.chat_id_,Abs)
else
EditMsg(Chat_Id2, Msg_Id2, "᥀︙عذرا صلاحية الامر منتهيه !") 
end
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local Help = DevAbs:get(Engineer..'Abs:Help')
local Text = [[
...
local Help = DevAbs:get(Engineer..'Abs:Help1')
local Text = [[
...
local Help = DevAbs:get(Engineer..'Abs:Help2')
local Text = [[
᥀︙اوامر الادمنيه ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
᥀︙الاعدادت
...
local Help = DevAbs:get(Engineer..'Abs:Help3')
local Text = [[
᥀︙اوامر المدراء ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
᥀︙فحص البوت
...
local Help = DevAbs:get(Engineer..'Abs:Help4')
local Text = [[
᥀︙اوامر المنشئين ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
᥀︙تنزيل الكل
...
local Help = DevAbs:get(Engineer..'Abs:Help5')
local Text = [[
᥀︙اوامر المطورين ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
᥀︙الكروبات
...
if tonumber(Abs) == tonumber(data.sender_user_id_) then
local Help = DevAbs:get(Engineer..'Abs:Help6')
local Text = [[
᥀︙اوامر الاعضاء ↫ ⤈
┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉
...
if text and DevAbs:get(Engineer.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
local NewCmmd = DevAbs:get(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
if NewCmmd then
DevAbs:del(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text)
DevAbs:del(Engineer.."Set:Cmd:Group:New"..msg.chat_id_)
DevAbs:srem(Engineer.."List:Cmd:Group:New"..msg.chat_id_,text)
...
if text and DevAbs:get(Engineer.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_) == "true" then
DevAbs:set(Engineer.."Set:Cmd:Group:New"..msg.chat_id_,text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙ارسل الامر الجديد", 1, 'html')
DevAbs:del(Engineer.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_)
DevAbs:set(Engineer.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_,"true1") 
return false
end
if text and DevAbs:get(Engineer.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_) == "true1" then
local NewCmd = DevAbs:get(Engineer.."Set:Cmd:Group:New"..msg.chat_id_)
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":"..text,NewCmd)
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الامر", 1, 'html')
DevAbs:del(Engineer.."Set:Cmd:Group1"..msg.chat_id_..":"..msg.sender_user_id_)
return false
end
if Constructor(msg) then
if text == "الاوامر المضافه" and ChCheck(msg) then
local List = DevAbs:smembers(Engineer.."List:Cmd:Group:New"..msg.chat_id_.."") 
t = "᥀︙قائمة الاوامر المضافه ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
Cmds = DevAbs:get(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
if Cmds then 
t = t..k.."~ ("..v..") • {"..Cmds.."}\n"
else
t = t..k.."~ ("..v..") \n"
...
local List = DevAbs:smembers(Engineer.."List:Cmd:Group:New"..msg.chat_id_)
for k,v in pairs(List) do
DevAbs:del(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":"..v)
DevAbs:del(Engineer.."List:Cmd:Group:New"..msg.chat_id_)
end
...
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":ا","ايدي")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"ا")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":م","رفع مميز")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"م")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":اد","رفع ادمن")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"اد")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":مد","رفع مدير")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"مد")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":من","رفع منشئ")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"من")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":اس","رفع منشئ اساسي")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"اس")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":مط","رفع مطور")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"مط")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":ثانوي","رفع مطور ثانوي")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"ثانوي")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":تك","تنزيل الكل")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"تك")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":تعط","تعطيل الايدي بالصوره")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"تعط")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":تفع","تفعيل الايدي بالصوره")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"تفع")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":ر","الرابط")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"ر")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":رر","ردود المدير")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"رر")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":،،","مسح المكتومين")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"،،")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":رد","اضف رد")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"رد")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":سح","مسح سحكاتي")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"سح")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":رس","مسح رسائلي")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"رس")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":غ","غنيلي")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"غ")
DevAbs:set(Engineer.."Set:Cmd:Group:New1"..msg.chat_id_..":#","مسح قائمه العام")
DevAbs:sadd(Engineer.."List:Cmd:Group:New"..msg.chat_id_,"#")
...
DevAbs:set(Engineer.."Set:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
...
DevAbs:set(Engineer.."Del:Cmd:Group"..msg.chat_id_..":"..msg.sender_user_id_,"true") 
...
local List = DevAbs:smembers(Engineer.."Coomds"..msg.chat_id_)
if #List == 0 then
...
for k,v in pairs(List) do
var = DevAbs:get(Engineer.."Comd:New:rt:Abs:"..v..msg.chat_id_)
if var then
t = t..k.."~ "..v.." • ("..var..")\n"
else
t = t..k.."~ "..v.."\n"
...
local List = DevAbs:smembers(Engineer.."Coomds"..msg.chat_id_)
for k,v in pairs(List) do
DevAbs:del(Engineer.."Comd:New:rt:Abs:"..v..msg.chat_id_)
DevAbs:del(Engineer.."Coomds"..msg.chat_id_)
end
...
DevAbs:set(Engineer.."Comd:New:rt"..msg.chat_id_..msg.sender_user_id_,ComdNew)  
DevAbs:sadd(Engineer.."Coomds"..msg.chat_id_,ComdNew)  
DevAbs:setex(Engineer.."Comd:New"..msg.chat_id_..msg.sender_user_id_,200,true)  
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙ارسل نوع الصلاحيه \n{ عضو • مميز  • ادمن  • مدير }\n᥀︙ارسل الغاء لالغاء الامر ", 1, 'html')
end
if text and text:match("^حذف صلاحيه (.*)$") and ChCheck(msg) or text and text:match("^مسح صلاحيه (.*)$") and ChCheck(msg) then 
ComdNew = text:match("^حذف صلاحيه (.*)$") or text:match("^مسح صلاحيه (.*)$")
DevAbs:del(Engineer.."Comd:New:rt:Abs:"..ComdNew..msg.chat_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم حذف الصلاحيه", 1, 'html')
end
if DevAbs:get(Engineer.."Comd:New"..msg.chat_id_..msg.sender_user_id_) then 
if text and text:match("^↫ الغاء ᥀$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء الامر", 1, 'html')
DevAbs:del(Engineer.."Comd:New"..msg.chat_id_..msg.sender_user_id_) 
return false  
end 
if text == "مدير" then
...
local textn = DevAbs:get(Engineer.."Comd:New:rt"..msg.chat_id_..msg.sender_user_id_)  
DevAbs:set(Engineer.."Comd:New:rt:Abs:"..textn..msg.chat_id_,text)
...
if DevAbs:sismember(Engineer.."Coomds"..msg.chat_id_,text1[2]) then
...
DevAbs:incr(Engineer..'Abs:UsersMsgs'..Engineer..os.date('%d')..':'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:incr(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:incr(Engineer..'Abs:MsgNumberDay'..msg.chat_id_..':'..os.date('%d'))  
ChatType = 'sp' 
elseif id:match("^(%d+)") then
if not DevAbs:sismember(Engineer.."Abs:Users",msg.chat_id_) then
DevAbs:sadd(Engineer.."Abs:Users",msg.chat_id_)
end
ChatType = 'pv' 
else
ChatType = 'gp' 
...
DevAbs:setex(Engineer..'Abs:Start:Time'..msg.sender_user_id_,300,true)
return false
end 
--     Source Engineer     --
...
DevAbs:sadd(Engineer..'Abs:Ban:Pv',data.id_)  
return false  
end 
if text == 'الغاء الحظر' or text == 'الغاء حظر' then
local Text = '᥀︙العضو ↫ ['..string.sub(data.first_name_,0, 40)..'](tg://user?id='..data.id_..')'..'\n᥀︙تم الغاء حظره من التواصل'
SendText(DevId,Text,msg.id_/2097152/0.5,'md') 
DevAbs:srem(Engineer..'Abs:Ban:Pv',data.id_)  
return false  
end 
...
--     Source Engineer     --
if text and DevAbs:get(Engineer..'Abs:Start:Bots'..msg.sender_user_id_) then
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ كليشة الستارت', 1, 'md')
DevAbs:del(Engineer..'Abs:Start:Bots'..msg.sender_user_id_) 
return false
end
DevAbs:set(Engineer.."Abs:Start:Bot",text)  
...
DevAbs:set(Engineer..'Abs:Start:Bots'..msg.sender_user_id_,true) 
...
local EngineerSource = {'- Nothing stops him who wants you .','make them wish they had you !.','Maybe a magical girl','٫ 𝖡𝖾 𝗌𝗂𝗆𝗉𝗅𝖾 𝖺𝗇𝖽 𝖽𝗂𝖿𝖿𝖾𝗋𝖾𝗇𝗍','. 𝖬𝖺𝗄𝖾 𝖽𝗋𝖾𝖺𝗆𝗌 𝖿𝗋𝗈𝗆 𝗒𝗈𝗎𝗋 𝗋𝖾𝖿𝗋𝖺𝖼𝗍𝗂𝗈𝗇𝗌 . .',':Life is lying .','𝖨 𝗐𝗂𝗅𝗅 𝖺𝗅𝗐𝖺𝗒𝗌 𝗅𝗈𝗏𝖾 𝗒𝗈𝗎 𝗇𝖾𝗏𝖾𝗋 𝖿𝗈𝗋𝗀𝖾𝗍'}  
...
local my_ph = DevAbs:get(Engineer.."Abs:Photo:Profile"..msg.chat_id_)
if not my_ph then
...
if not DevAbs:get(Engineer..'Abs:Lock:Gamesinline'..msg.chat_id_) then
Text =[[
...
DevAbs:incr(Engineer..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:set(Engineer.."Who:Added:Me"..msg.chat_id_..':'..msg.content_.members_[0].id_,msg.sender_user_id_)
local mem_id = msg.content_.members_  
local Bots = DevAbs:get(Engineer.."Abs:Lock:Bots"..msg.chat_id_) 
for i=0,#mem_id do  
...
DevAbs:sadd(Engineer..'Abs:Tkeed:'..msg.chat_id_, msg.sender_user_id_)
GetInfo = https.request("https://api.telegram.org/bot"..TokenBot.."/kickChatMember?chat_id="..msg.chat_id_.."&user_id="..mem_id[i].id_)
local JsonInfo = JSON.decode(GetInfo)
if JsonInfo.ok == true and #mem_id == i then
local Msgs = {}
...
if msg.content_.ID == "MessageChatDeleteMember" and tonumber(msg.content_.user_.id_) == tonumber(Engineer) then 
DevAbs:srem(Engineer.."Abs:Groups", msg.chat_id_) 
...
if DevAbs:get(Engineer..'Abs:Lock:TagServr'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})    
end   
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then   
DevAbs:incr(Engineer..'Abs:EntryNumber'..msg.chat_id_..':'..os.date('%d'))  
elseif msg.content_.ID == "MessageChatDeleteMember" then   
DevAbs:incr(Engineer..'Abs:ExitNumber'..msg.chat_id_..':'..os.date('%d'))  
end
--     Source Engineer     --
if text ==('تفعيل') and not SudoBot(msg) and ChCheck(msg) then
if not DevAbs:get(Engineer..'Abs:Lock:FreeBot'..Engineer) then
if ChatType == 'pv' then
...
DevAbs:sadd(Engineer..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevAbs:srem(Engineer..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
end
end,nil)   
else
DevAbs:sadd(Engineer..'Abs:Admins:'..msg.chat_id_, admins[i].user_id_)
...
DevAbs:sadd(Engineer.."Abs:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevAbs:sadd(Engineer.."Abs:AbsConstructor:"..msg.chat_id_,admins[i].user_id_)
tdcli_function ({ID = "GetUser",user_id_ = admins[i].user_id_},function(arg,ba) 
if ba.first_name_ == false then
DevAbs:srem(Engineer.."Abs:BasicConstructor:"..msg.chat_id_,admins[i].user_id_)
DevAbs:srem(Engineer.."Abs:AbsConstructor:"..msg.chat_id_,admins[i].user_id_)
end
end,nil)  
end 
end
end,nil)
if DevAbs:sismember(Engineer..'Abs:Groups',msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙المجموعه بالتاكيد مفعله', 1, 'md')
else
if tonumber(data.member_count_) < tonumber(DevAbs:get(Engineer..'Abs:Num:Add:Bot') or 0) and not SecondSudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙عدد اعضاء المجموعه اقل من ↫ *'..(DevAbs:get(Engineer..'Abs:Num:Add:Bot') or 0)..'* عضو', 1, 'md')
return false
end
ReplyStatus(msg,result.id_,"ReplyBy","᥀︙تم تفعيل المجموعه "..dp.title_)  
DevAbs:sadd(Engineer.."Abs:Groups",msg.chat_id_)
DevAbs:sadd(Engineer..'Abs:BasicConstructor:'..msg.chat_id_,msg.sender_user_id_)
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
local Name1 = Name1:gsub("`","") 
...
DevAbs:set(Engineer.."Abs:Groups:Links"..msg.chat_id_,LinkGroup) 
...
DevAbs:set(Engineer..'Save:UserName'..msg.sender_user_id_,data.username_)
end;end,nil) 
--     Source Engineer     --
local ReFalse = tostring(msg.chat_id_)
if not DevAbs:sismember(Engineer.."Abs:Groups",msg.chat_id_) and not ReFalse:match("^(%d+)") and not SudoBot(msg) then
...
if DevAbs:get(Engineer..'Abs:Lock:Robot'..msg.chat_id_) then
tdcli_function({ID="GetUser",user_id_=msg.sender_user_id_},function(arg,dp) 
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id="..dp.id_)
DevAbs:sadd(Engineer..'Abs:Tkeed:'..msg.chat_id_, dp.id_)
...
if DevAbs:get(Engineer.."Abs:Lock:Join"..msg.chat_id_) then
...
if not Manager(msg) then 
local filter = DevAbs:smembers(Engineer.."Abs:FilterPhoto"..msg.chat_id_)
...
if not Manager(msg) then 
local filter = DevAbs:smembers(Engineer.."Abs:FilterAnimation"..msg.chat_id_)
for k,v in pairs(filter) do
...
if not Manager(msg) then 
local filter = DevAbs:smembers(Engineer.."Abs:FilterSteckr"..msg.chat_id_)
for k,v in pairs(filter) do
...
local DelGpRedRedods = DevAbs:get(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
local GetGpTexts = DevAbs:get(Engineer..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
if DelGpRedRedods == 'DelGpRedRedods' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'᥀︙الرد ↫ '..msg.content_.text_..' للكلمه ↫ '..GetGpTexts..' تم حذفها',  1, "html")
DevAbs:del(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:srem(Engineer..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelGpRed = DevAbs:get(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
if DelGpRed == 'DelGpRedod' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'᥀︙الكلمه ↫ '..msg.content_.text_..' تم حذفها',  1, "html")
DevAbs:del(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Text:GpTexts'..msg.content_.text_..msg.chat_id_)
DevAbs:srem(Engineer..'Abs:Manager:GpRedod'..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelGpRed = DevAbs:get(Engineer..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
if DelGpRed == 'DelGpRed' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'᥀︙الكلمه ↫ '..msg.content_.text_..' تم حذفها',  1, "html")
DevAbs:del(Engineer..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Engineer..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_)
DevAbs:srem(Engineer..'Abs:Manager:GpRed'..msg.chat_id_,msg.content_.text_)
return false
end
end
if text and text:match("^(.*)$") then
local DelAllRed = DevAbs:get(Engineer.."Abs:Add:AllRed"..msg.sender_user_id_)
if DelAllRed == 'DelAllRed' then
Dev_Abs(msg.chat_id_, msg.id_, 1,'᥀︙الكلمه ↫ '..msg.content_.text_..' تم حذفها',  1, "html")
DevAbs:del(Engineer.."Abs:Add:AllRed"..msg.sender_user_id_)
DevAbs:del(Engineer.."Abs:Gif:AllRed"..msg.content_.text_)
DevAbs:del(Engineer.."Abs:Voice:AllRed"..msg.content_.text_)
DevAbs:del(Engineer.."Abs:Audio:AllRed"..msg.content_.text_)
DevAbs:del(Engineer.."Abs:Photo:AllRed"..msg.content_.text_)
DevAbs:del(Engineer.."Abs:Stecker:AllRed"..msg.content_.text_)
DevAbs:del(Engineer.."Abs:Video:AllRed"..msg.content_.text_)
DevAbs:del(Engineer.."Abs:File:AllRed"..msg.content_.text_)
DevAbs:del(Engineer.."Abs:Text:AllRed"..msg.content_.text_)
DevAbs:del(Engineer.."Abs:Sudo:AllRed",msg.content_.text_)
return false
end
end
--     Source Engineer     --
if text and text:match("^(.*)$") then
local SaveGpRedod = DevAbs:get(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
if SaveGpRedod == 'SaveGpRedod' then
local GetGpTexts = DevAbs:get(Engineer..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
local List = DevAbs:smembers(Engineer..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_)
if text == "الغاء" then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙᥀︙تم الغاء عملية حفظ الردود المتعدده للامر ↫ "..GetGpTexts ,  1, "md")
DevAbs:del(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_)
DevAbs:srem(Engineer..'Abs:Manager:GpRedod'..msg.chat_id_,GetGpTexts)
return false
end
Text = text:gsub('"',""):gsub('"',""):gsub("`",""):gsub("*","")
DevAbs:sadd(Engineer..'Abs:Text:GpTexts'..GetGpTexts..msg.chat_id_,Text)
if #List == 4 then 
...
if text and not DevAbs:get(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_) then
if DevAbs:sismember(Engineer..'Abs:Manager:GpRedod'..msg.chat_id_,text) then
local EngineerSource =  DevAbs:smembers(Engineer..'Abs:Text:GpTexts'..text..msg.chat_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '['..EngineerSource[math.random(#EngineerSource)]..']' , 1, 'md')  
end
end
--     Source Engineer     --
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then 
local SaveGpRed = DevAbs:get(Engineer..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
if SaveGpRed == 'SaveGpRed' then 
if text == 'الغاء' then
local DelManagerRep = DevAbs:get(Engineer..'DelManagerRep'..msg.chat_id_)
DevAbs:srem(Engineer..'Abs:Manager:GpRed'..msg.chat_id_,DelManagerRep)
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ الرد', 1, 'md')
DevAbs:del(Engineer..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
DevAbs:del(Engineer..'DelManagerRep'..msg.chat_id_)
return false
end
DevAbs:del(Engineer..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
local SaveGpRed = DevAbs:get(Engineer..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_)
if msg.content_.video_ then DevAbs:set(Engineer..'Abs:Video:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then DevAbs:set(Engineer..'Abs:File:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then DevAbs:set(Engineer..'Abs:Stecker:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.sticker_.sticker_.persistent_id_) 
end 
if msg.content_.voice_ then DevAbs:set(Engineer..'Abs:Voice:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.voice_.voice_.persistent_id_) 
end
if msg.content_.audio_ then DevAbs:set(Engineer..'Abs:Audio:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.audio_.audio_.persistent_id_) 
end
...
DevAbs:set(Engineer..'Abs:Photo:GpRed'..SaveGpRed..msg.chat_id_, photo_in_group) 
end
if msg.content_.animation_ then DevAbs:set(Engineer..'Abs:Gif:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.animation_.animation_.persistent_id_) 
end 
if msg.content_.text_ then
DevAbs:set(Engineer..'Abs:Text:GpRed'..SaveGpRed..msg.chat_id_, msg.content_.text_)
end 
...
if msg.content_.text_ and not DevAbs:get(Engineer..'Abs:Lock:GpRed'..msg.chat_id_) then 
if DevAbs:get(Engineer..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Engineer..'Abs:Video:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(Engineer..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Engineer..'Abs:File:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(Engineer..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Engineer..'Abs:Voice:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(Engineer..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Engineer..'Abs:Audio:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(Engineer..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Engineer..'Abs:Photo:GpRed'..msg.content_.text_..msg.chat_id_)) 
end
if DevAbs:get(Engineer..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Engineer..'Abs:Gif:GpRed'..msg.content_.text_..msg.chat_id_)) 
end 
if DevAbs:get(Engineer..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_) then 
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Engineer..'Abs:Stecker:GpRed'..msg.content_.text_..msg.chat_id_))
end
if DevAbs:get(Engineer..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_) then
function EngineerSource(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevAbs:get(Engineer..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevAbs:get(Engineer..'Abs:Text:GpRed'..msg.content_.text_..msg.chat_id_)
...
text = msg.content_.text_
if msg.content_.text_ or msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.voice_ or msg.content_.audio_ or msg.content_.photo_ or msg.content_.animation_ then
local SaveAllRed = DevAbs:get(Engineer.."Abs:Add:AllRed"..msg.sender_user_id_)
if SaveAllRed == 'SaveAllRed' then
if text == 'الغاء' then
local DelSudoRep = DevAbs:get(Engineer..'DelSudoRep')
DevAbs:del(Engineer.."Abs:Sudo:AllRed",DelSudoRep)
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ الرد', 1, 'md')
DevAbs:del(Engineer.."Abs:Add:AllText"..msg.sender_user_id_)
DevAbs:del(Engineer.."Abs:Add:AllRed"..msg.sender_user_id_)
DevAbs:del(Engineer.."DelSudoRep")
return false
end
DevAbs:del(Engineer.."Abs:Add:AllRed"..msg.sender_user_id_)
local SaveAllRed = DevAbs:get(Engineer.."Abs:Add:AllText"..msg.sender_user_id_)
if msg.content_.video_ then
DevAbs:set(Engineer.."Abs:Video:AllRed"..SaveAllRed, msg.content_.video_.video_.persistent_id_)
end
if msg.content_.document_ then
DevAbs:set(Engineer.."Abs:File:AllRed"..SaveAllRed, msg.content_.document_.document_.persistent_id_)
end
if msg.content_.sticker_ then
DevAbs:set(Engineer.."Abs:Stecker:AllRed"..SaveAllRed, msg.content_.sticker_.sticker_.persistent_id_)
end
if msg.content_.voice_ then
DevAbs:set(Engineer.."Abs:Voice:AllRed"..SaveAllRed, msg.content_.voice_.voice_.persistent_id_)
end
if msg.content_.audio_ then
DevAbs:set(Engineer.."Abs:Audio:AllRed"..SaveAllRed, msg.content_.audio_.audio_.persistent_id_)
end
if msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
...
DevAbs:set(Engineer.."Abs:Photo:AllRed"..SaveAllRed, photo_in_all_groups)
end
if msg.content_.animation_ then
DevAbs:set(Engineer.."Abs:Gif:AllRed"..SaveAllRed, msg.content_.animation_.animation_.persistent_id_)
end
if msg.content_.text_ then
DevAbs:set(Engineer.."Abs:Text:AllRed"..SaveAllRed, msg.content_.text_)
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم حفظ الرد الجديد', 1, 'md') 
DevAbs:del(Engineer.."Abs:Add:AllText"..msg.sender_user_id_)
DevAbs:del(Engineer..'DelSudoRep')
return false end end
if msg.content_.text_ and not DevAbs:get(Engineer..'Abs:Lock:AllRed'..msg.chat_id_) then
if DevAbs:get(Engineer.."Abs:Video:AllRed"..msg.content_.text_) then
sendVideo(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Engineer.."Abs:Video:AllRed"..msg.content_.text_))
end
if DevAbs:get(Engineer.."Abs:File:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Engineer.."Abs:File:AllRed"..msg.content_.text_))
end
if DevAbs:get(Engineer.."Abs:Voice:AllRed"..msg.content_.text_)  then
sendVoice(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Engineer.."Abs:Voice:AllRed"..msg.content_.text_))
end
if DevAbs:get(Engineer.."Abs:Audio:AllRed"..msg.content_.text_)  then
sendAudio(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Engineer.."Abs:Audio:AllRed"..msg.content_.text_))
end
if DevAbs:get(Engineer.."Abs:Photo:AllRed"..msg.content_.text_)  then
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Engineer.."Abs:Photo:AllRed"..msg.content_.text_))
end
if  DevAbs:get(Engineer.."Abs:Gif:AllRed"..msg.content_.text_) then
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, DevAbs:get(Engineer.."Abs:Gif:AllRed"..msg.content_.text_))
end
if DevAbs:get(Engineer.."Abs:Stecker:AllRed"..msg.content_.text_) then
sendSticker(msg.chat_id_, msg.id_, 0, 1,nil, DevAbs:get(Engineer.."Abs:Stecker:AllRed"..msg.content_.text_))
end
if DevAbs:get(Engineer.."Abs:Text:AllRed"..msg.content_.text_) then
function EngineerSource(extra,result,success)
if result.username_ then username = '[@'..result.username_..']' else username = 'لا يوجد' end
local edit_msg = DevAbs:get(Engineer..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local Text = DevAbs:get(Engineer.."Abs:Text:AllRed"..msg.content_.text_)
local Text = Text:gsub('#username',(username or 'لا يوجد')) 
local Text = Text:gsub('#name','['..result.first_name_..']')
local Text = Text:gsub('#id',msg.sender_user_id_)
local Text = Text:gsub('#edit',edit_msg)
...
if Type == "keed" and not DevAbs:sismember(Engineer..'Abs:Tkeed:'..msg.chat_id_, msg.sender_user_id_) then
...
if Type == "mute" and not DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_, msg.sender_user_id_) then
DevAbs:sadd(Engineer..'Abs:Muted:'..msg.chat_id_,msg.sender_user_id_)
my_ide = msg.sender_user_id_
msgm = msg.id_
local num = 100
for i=1,tonumber(num) do
...
--  end functions Engineer --
--     Source Engineer     --
--       Spam Check       --
if not VipMem(msg) and msg.content_.ID ~= "MessageChatAddMembers" and DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") then 
if msg.sender_user_id_ ~= Engineer then
floods = DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") or "nil"
Num_Msg_Max = DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
Time_Spam = DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 5
local post_count = tonumber(DevAbs:get(Engineer.."Abs:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_) or 0)
if post_count > tonumber(DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5) then 
local ch = msg.chat_id_
local type = DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") 
NotSpam(msg,type)  
end
DevAbs:setex(Engineer.."Abs:Spam:Cont"..msg.sender_user_id_..":"..msg.chat_id_, tonumber(DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") or 3), post_count+1) 
local edit_id = data.text_ or "nil"  
Num_Msg_Max = 5
if DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") then
Num_Msg_Max = DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") 
end
if DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") then
Time_Spam = DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam:Time") 
end 
end
end 
--     Source Engineer     --
...
if Constructor(msg) or tonumber(msg.sender_user_id_) == tonumber(Engineer) then
DevAbs:set(Engineer..'Abs:PinnedMsg'..msg.chat_id_,msg.content_.message_id_)
else
local pin_id = DevAbs:get(Engineer..'Abs:PinnedMsg'..msg.chat_id_)
if pin_id and DevAbs:get(Engineer..'Abs:Lock:Pin'..msg.chat_id_) then
pinmsg(msg.chat_id_,pin_id,0)
end
end
end
if DevAbs:get(Engineer..'Abs:viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
DevAbs:del(Engineer..'Abs:viewget'..msg.sender_user_id_)
else
...
if DevAbs:get(Engineer..'Abs:Lock:Forwards'..msg.chat_id_) then
if msg.forward_info_.ID == "MessageForwardedFromUser" or msg.forward_info_.ID == "MessageForwardedPost" then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
end
if DevAbs:get(Engineer..'Abs:Lock:Photo'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
if DevAbs:get(Engineer..'Abs:Lock:Links'..msg.chat_id_) then
if msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or msg.content_.caption_:match("[Tt].[Mm][Ee]") or msg.content_.caption_:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Dd][Oo][Gg]") then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if DevAbs:get(Engineer..'Abs:Lock:Tags'..msg.chat_id_) then
if msg.content_.caption_:match("@") then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("#") then
if DevAbs:get(Engineer..'Abs:Lock:Hashtak'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[Hh][Tt][Tt][Pp][Ss]://") or msg.content_.caption_:match("[Hh][Tt][Tt][Pp]://") or msg.content_.caption_:match(".[Ii][Rr]") or msg.content_.caption_:match(".[Cc][Oo][Mm]") or msg.content_.caption_:match(".[Oo][Rr][Gg]") or msg.content_.caption_:match(".[Ii][Nn][Ff][Oo]") or msg.content_.caption_:match("[Ww][Ww][Ww].") or msg.content_.caption_:match(".[Xx][Yy][Zz]") or msg.content_.caption_:match(".[Tt][Kk]") or msg.content_.ID == "MessageEntityTextUrl" or msg.content_.ID == "MessageEntityUrl" then
if DevAbs:get(Engineer..'Abs:Lock:WebLinks'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[\216-\219][\128-\191]") then
if DevAbs:get(Engineer..'Abs:Lock:Arabic'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
if msg.content_.caption_:match("[A-Z]") or msg.content_.caption_:match("[a-z]") then
if DevAbs:get(Engineer..'Abs:Lock:English'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
end
...
if DevAbs:get(Engineer..'Abs:Lock:Markdown'..msg.chat_id_) then
if not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
if DevAbs:get(Engineer..'Abs:Lock:Document'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if not VipMem(msg) then
if DevAbs:get(Engineer..'Abs:Lock:Inline'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
--     Source Engineer     --
...
elseif msg.content_.ID == "MessageSticker" then
if not VipMem(msg) then
if DevAbs:get(Engineer..'Abs:Lock:Stickers'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
if DevAbs:get(Engineer..'Abs:Groups:Welcomes'..msg.chat_id_) then
Welcomes = DevAbs:get(Engineer..'Abs:Groups:Welcomes'..msg.chat_id_)
else
Welcomes = '• نورت حبي \n• firstname \n• username'
end
...
if not DevAbs:get(Engineer..'Abs:Lock:BotWelcome') then 
tdcli_function ({ID = "GetUserProfilePhotos",user_id_ = Engineer,offset_ = 0,limit_ = 1},function(extra,Abs,success) 
for i=0,#msg.content_.members_ do    
BotWelcome = msg.content_.members_[i].id_    
if BotWelcome and BotWelcome == tonumber(Engineer) then 
if DevAbs:sismember(Engineer..'Abs:Groups',msg.chat_id_) then BotText = "مفعله في السابق\n᥀︙ارسل ↫ الاوامر واستمتع بالمميزيات" else BotText = "معطله يجب رفعي مشرف\n᥀︙بعد ذلك يرجى ارسال امر ↫ تفعيل\n᥀︙سيتم رفع الادمنيه والمنشئ تلقائيا" end 
if DevAbs:get(Engineer.."Abs:Text:BotWelcome") then AbsText = DevAbs:get(Engineer.."Abs:Text:BotWelcome") else AbsText = "᥀︙مرحبا انا بوت اسمي "..NameBot.."\n᥀︙حالة المجموعه ↫ "..BotText.."\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉" end 
if DevAbs:get(Engineer.."Abs:Photo:BotWelcome") then AbsPhoto = DevAbs:get(Engineer.."Abs:Photo:BotWelcome") elseif Abs.photos_[0] then AbsPhoto = Abs.photos_[0].sizes_[1].photo_.persistent_id_ else AbsPhoto = nil end 
...
if DevAbs:get(Engineer..'Abs:Lock:Contact'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
--     Source Engineer     --
...
if DevAbs:get(Engineer..'Abs:Lock:Music'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(Engineer..'Abs:Lock:Voice'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(Engineer..'Abs:Lock:Location'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
return
end
if msg.content_.caption_ then
...
if DevAbs:get(Engineer..'Abs:Lock:Videos'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(Engineer..'Abs:Lock:Gifs'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
if msg.content_.caption_ then
Filters(msg, msg.content_.caption_)
...
if DevAbs:get(Engineer..'Abs:Lock:Text'..msg.chat_id_) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
...
if not DevAbs:get(Engineer..'Abs:Spam:Text'..msg.chat_id_) then
sens = 400
else
sens = tonumber(DevAbs:get(Engineer..'Abs:Spam:Text'..msg.chat_id_))
end
if DevAbs:get(Engineer..'Abs:Lock:Spam'..msg.chat_id_) and string.len(msg.content_.text_) > (sens) or ctrl_chars > (sens) or real_digits > (sens) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
end
end
...
--     Source Engineer     --
if DevAbs:get(Engineer.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_) then
if text == "الغاء" then
send(msg.chat_id_,msg.id_,"᥀︙تم الغاء حفظ الرابط")       
DevAbs:del(Engineer.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_) 
return false
end
if msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)") then
local Link = msg.content_.text_:match("(https://telegram.me/joinchat/%S+)") or msg.content_.text_:match("(https://t.me/joinchat/%S+)")
DevAbs:set(Engineer.."Abs:Groups:Links"..msg.chat_id_,Link)
...
if DevAbs:get('EngineerSource:'..Engineer.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
DevAbs:del('EngineerSource:'..Engineer..'id:user'..msg.chat_id_)  
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء الامر', 1, 'md')
DevAbs:del('EngineerSource:'..Engineer.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del('EngineerSource:'..Engineer.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = DevAbs:get('EngineerSource:'..Engineer..'id:user'..msg.chat_id_)  
DevAbs:incrby(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..iduserr,numadded)
Dev_Abs(msg.chat_id_, msg.id_,  1, "᥀︙تم اضافة "..numadded..' رساله', 1, 'md')
DevAbs:del('EngineerSource:'..Engineer..'id:user'..msg.chat_id_) 
end
end
if text and Constructor(msg) then 
if DevAbs:get('EngineerSource:'..Engineer.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
DevAbs:del('EngineerSource:'..Engineer..'ids:user'..msg.chat_id_)  
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء الامر', 1, 'md')
DevAbs:del('EngineerSource:'..Engineer.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del('EngineerSource:'..Engineer.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local numadded = string.match(text, "(%d+)") 
local iduserr = DevAbs:get('EngineerSource:'..Engineer..'ids:user'..msg.chat_id_)  
DevAbs:incrby(Engineer..'Abs:GamesNumber'..msg.chat_id_..iduserr,numadded)  
...
if not DevAbs:get(Engineer.."Abs:Lock:Fshar"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
if DevAbs:get(Engineer.."Abs:Lock:Farsi"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
ReplyStatus(msg,msg.sender_user_id_,"WrongWay","᥀︙ممنوع التكلم بالغه الفارسيه هنا")  
end end
if text and (text:match("ڬ") or text:match("ٺ") or text:match("چ") or text:match("ڇ") or text:match("ڿ") or text:match("ڀ") or text:match("ڎ") or text:match("ݫ") or text:match("ژ") or text:match("ڟ") or text:match("ݜ") or text:match("ڸ") or text:match("پ") or text:match("۴") or text:match("مک") or text:match("زدن") or text:match("دخترا") or text:match("دیوث") or text:match("کلیپشن") or text:match("خوششون") or text:match("میدا") or text:match("که") or text:match("بدانیم") or text:match("باید") or text:match("زناشویی") or text:match("آموزش") or text:match("راحتی") or text:match("خسته") or text:match("بیام") or text:match("بپوشم") or text:match("كرمه")) then
if DevAbs:get(Engineer.."Abs:Lock:FarsiBan"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
ChatKick(msg.chat_id_, msg.sender_user_id_)
end end 
if text and (text:match("خره بالله") or text:match("خبربك") or text:match("كسدينربك") or text:match("خرب بالله") or text:match("خرب الله") or text:match("خره بربك") or text:match("الله الكواد") or text:match("خره بمحمد") or text:match("كسم الله") or text:match("كسم ربك") or text:match("كسربك") or text:match("كسختالله") or text:match("كسخت الله") or text:match("خره بدينك") or text:match("خرهبدينك") or text:match("كسالله") or text:match("خربالله")) then
if not DevAbs:get(Engineer.."Abs:Lock:Kfr"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
if not DevAbs:get(Engineer.."Abs:Lock:Taf"..msg.chat_id_) and not VipMem(msg) then
DeleteMessage(msg.chat_id_,{[0] = msg.id_})
...
Welcomes = DevAbs:get(Engineer..'Abs:Groups:Welcomes'..v) or ''
Welcomes = Welcomes:gsub('"',"") Welcomes = Welcomes:gsub("'","") Welcomes = Welcomes:gsub(",","") Welcomes = Welcomes:gsub("*","") Welcomes = Welcomes:gsub(";","") Welcomes = Welcomes:gsub("`","") Welcomes = Welcomes:gsub("{","") Welcomes = Welcomes:gsub("}","") 
AbsConstructors = DevAbs:smembers(Engineer..'Abs:AbsConstructor:'..v)
Constructors = DevAbs:smembers(Engineer..'Abs:BasicConstructor:'..v)
BasicConstructors = DevAbs:smembers(Engineer..'Abs:Constructor:'..v)
Managers = DevAbs:smembers(Engineer..'Abs:Managers:'..v)
Admis = DevAbs:smembers(Engineer..'Abs:Admins:'..v)
Vips = DevAbs:smembers(Engineer..'Abs:VipMem:'..v)
if k == 1 then
GetJson = GetJson..'"'..v..'":{'
else
GetJson = GetJson..',"'..v..'":{'
...
GetJson = GetJson..'}}'
local File = io.open('./'..Engineer..'.json', "w")
File:write(GetJson)
File:close()
sendDocument(msg.chat_id_, msg.id_, 0, 1, nil, './'..Engineer..'.json', '᥀︙يحتوي الملف على ↫ '..#List..' مجموعه',dl_cb, nil)
io.popen('rm -rf ./'..Engineer..'.json')
end
...
--     Source Engineer     --
if DevAbs:get(Engineer.."SET:GAME"..msg.chat_id_) then  
if text and text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
if tonumber(NUM) > 6 then
Dev_Abs( msg.chat_id_, msg.id_, 1,"᥀︙يوجد فقط ( 6 ) اختيارات\n᥀︙ارسل اختيارك مره اخرى", 1, "md")    
return false  end 
local GETNUM = DevAbs:get(Engineer.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
DevAbs:del(Engineer.."SET:GAME"..msg.chat_id_)   
...
if DevAbs:get(Engineer..'DevAbs4'..msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
send(msg.chat_id_, msg.id_, "᥀︙تم الغاء الامر")
DevAbs:del(Engineer..'DevAbs4'..msg.sender_user_id_)
return false  end 
DevAbs:del(Engineer..'DevAbs4'..msg.sender_user_id_)
...
DevAbs:set(Engineer..'Abs:ChId',data.id_)
else
...
--     Source Engineer     --
if DevAbs:get(Engineer.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
DevAbs:del(Engineer.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء الامر', 1, 'md')
return false 
end 
DevAbs:del(Engineer.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_)
local DevText = msg.content_.text_:match("(.*)")
DevAbs:set(Engineer.."DevText", DevText)
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ كليشة المطور", 1, "md")
end
if DevAbs:get(Engineer..'Abs:NameBot'..msg.sender_user_id_) == 'msg' then
if text and text:match("^الغاء$") then 
DevAbs:del(Engineer..'Abs:NameBot'..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء الامر', 1, 'md')
return false 
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم حفظ اسم البوت ', 1, 'html')
DevAbs:del(Engineer..'Abs:NameBot'..msg.sender_user_id_)
DevAbs:set(Engineer..'Abs:NameBot', text)
return false 
end
--     Source Engineer     --
if text == "الرابط" and ChCheck(msg) then
if not DevAbs:get(Engineer.."Abs:Lock:GpLinks"..msg.chat_id_) then 
if DevAbs:get(Engineer.."Abs:Groups:Links"..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙𝖦𝗋𝗈𝗎𝗉 𝖫𝗂𝗇𝗄 ↬ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"..DevAbs:get(Engineer.."Abs:Groups:Links"..msg.chat_id_), 1, "html")
else 
...
if tonumber((DevAbs:get(Engineer..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1,'᥀︙لم تربح اي نقطه\n᥀︙ارسل ↫ الالعاب للعب', 1, 'md')
else 
Dev_Abs(msg.chat_id_, msg.id_, 1,'᥀︙عدد النقاط التي ربحتها ↫ '..(DevAbs:get(Engineer..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_)), 1, 'md')
end
end
if text ==  'حذف رسائلي' and ChCheck(msg) or text ==  'مسح رسائلي' and ChCheck(msg) then DevAbs:del(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم حذف جميع رسائلك', 1, 'md') end
if text ==  'حذف نقاطي' and ChCheck(msg) or text ==  'مسح نقاطي' and ChCheck(msg) then DevAbs:del(Engineer..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم حذف جميع نقاطك', 1, 'md') end
--     Source Engineer     --
...
DevAbs0 = (DevAbs:get(Engineer..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) * 50)
DevAbs:incrby(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_,DevAbs0)
Dev_Abs(msg.chat_id_, msg.id_, 1,'᥀︙تم بيع '..(DevAbs:get(Engineer..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_))..' من نقاطك\n᥀︙كل نقطه تساوي 50 رساله', 'md')
DevAbs:del(Engineer..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_)
end
end
--     Source Engineer     --
...
DevAbs:sadd(Engineer..'Abs:BasicConstructor:'..msg.chat_id_,Manager_id)  
DevAbs:sadd(Engineer..'Abs:AbsConstructor:'..msg.chat_id_,Manager_id)   
end  
end  
if num == 0 then
...
if DevAbs:get(Engineer.."Abs:Left:Bot"..Engineer) then
...
ChatLeave(msg.chat_id_, Engineer)
DevAbs:srem(Engineer.."Abs:Groups",msg.chat_id_)
end
--     Source Engineer     --
...
local msguser = tonumber(DevAbs:get(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_))
local user_msgs = DevAbs:get(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local cont = (tonumber(DevAbs:get(Engineer..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
local user_nkt = tonumber(DevAbs:get(Engineer..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)
if result.username_ then username = '@'..result.username_ else username = 'لا يوجد' end
...
DevAbs:setex(Engineer..'DevAbs4'..msg.sender_user_id_,360,true)
...
function EngineerSource(extra,result,success)
if DevAbs:get(Engineer.."Abs:Send:Bot"..Engineer) and not SecondSudo(msg) then 
send(msg.chat_id_, msg.id_,"᥀︙الاذاعه معطله من قبل المطور الاساسي")
return false
end
local GpList = DevAbs:smembers(Engineer.."Abs:Groups")
for k,v in pairs(GpList) do
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = result.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end
local PvList = DevAbs:smembers(Engineer.."Abs:Users")
...
if DevAbs:get(Engineer.."Abs:Kick:Me"..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙عذرا هذه الخاصيه معطله ', 1, 'md')
return false
end
DevAbs:set(Engineer..'yes'..msg.sender_user_id_, 'delyes')
DevAbs:set(Engineer..'no'..msg.sender_user_id_, 'delno')
...
DevAbs:set(Engineer.."Abs:Kick:Me"..msg.chat_id_, true)
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل امر اطردني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
end
if text == 'تفعيل اطردني' and Manager(msg) and ChCheck(msg) then
DevAbs:del(Engineer.."Abs:Kick:Me"..msg.chat_id_)
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل امر اطردني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
end
--     Source Engineer     --
if text == "نزلني" and ChCheck(msg) then
if DevAbs:get(Engineer.."Abs:Del:Me"..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙عذرا هذه الخاصيه معطله ', 1, 'md')
return false
end
DevAbs:set(Engineer..'yesdel'..msg.sender_user_id_, 'delyes')
DevAbs:set(Engineer..'nodel'..msg.sender_user_id_, 'delno')
...
DevAbs:set(Engineer.."Abs:Del:Me"..msg.chat_id_, true)
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل امر نزلني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
end
if text == 'تفعيل نزلني' and BasicConstructor(msg) and ChCheck(msg) then
DevAbs:del(Engineer.."Abs:Del:Me"..msg.chat_id_)
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل امر نزلني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
end
--     Source Engineer     --
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:TagAll'..msg.chat_id_)
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Lock:TagAll'..msg.chat_id_,true)
end
if Admin(msg) then
if text == "تاك للكل" and ChCheck(msg) then
if not DevAbs:get(Engineer..'Abs:Lock:TagAll'..msg.chat_id_) then
function TagAll(dp1,dp2)
local text = "᥀︙وينكم يالربع \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
i = 0
for k, v in pairs(dp2.members_) do
i = i + 1
if DevAbs:get(Engineer..'Save:UserName'..v.user_id_) then
text = text..i.."~ : [@"..DevAbs:get(Engineer..'Save:UserName'..v.user_id_).."]\n"
else
...
local EntryNumber = (DevAbs:get(Engineer..'Abs:EntryNumber'..msg.chat_id_..':'..os.date('%d')) or 0)
local ExitNumber = (DevAbs:get(Engineer..'Abs:ExitNumber'..msg.chat_id_..':'..os.date('%d')) or 0)
local MsgNumberDay = (DevAbs:get(Engineer..'Abs:MsgNumberDay'..msg.chat_id_..':'..os.date('%d')) or 0)
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل تفاعلي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:msg:Abs'..msg.chat_id_) 
end
if text == "تفعيل تفاعلي" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل تفاعلي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:msg:Abs'..msg.chat_id_,true)  
end
if DevAbs:get(Engineer.."Abs:msg:Abs"..msg.chat_id_) then
if msg.content_.ID then
get_msg = DevAbs:get(Engineer.."Abs:msg:Abs"..msg.sender_user_id_..":"..msg.chat_id_) or 0
gms = get_msg + 1
DevAbs:setex(Engineer..'Abs:msg:Abs'..msg.sender_user_id_..":"..msg.chat_id_,86400,gms)
end
...
get_msg = DevAbs:get(Engineer.."Abs:msg:Abs"..result.sender_user_id_..":"..msg.chat_id_) or 0
...
--     Source Engineer     --
if text == "جهاتي" and ChCheck(msg) or text == "اضافاتي" and ChCheck(msg) then add = (tonumber(DevAbs:get(Engineer..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0) Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙عدد جهاتك المضافه ↫ *❨ "..add.." ❩* ", 1, 'md') end
if text == "تعديلاتي" or text == "سحكاتي" and ChCheck(msg) then local edit_msg = DevAbs:get(Engineer..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0  Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙عدد تعديلاتك ↫ *❨ "..edit_msg.." ❩* ", 1, 'md') end
...
if text == 'مسح سحكاتي' or text == 'مسح تعديلاتي' or text == 'حذف سحكاتي' or text == 'حذف تعديلاتي' then DevAbs:del(Engineer..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم حذف جميع تعديلاتك بنجاح' , 1, 'md') end
if text == 'مسح جهاتي' or text == 'مسح اضافاتي' or text == 'حذف جهاتي' or text == 'حذف اضافاتي' then DevAbs:del(Engineer..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_) Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم حذف جميع جهاتك المضافه' , 1, 'md') end
--     Source Engineer     --
if text and text:match('^هينه @(.*)') and ChCheck(msg) or text and text:match('^هينها @(.*)') and ChCheck(msg) then 
if not DevAbs:get(Engineer..'Abs:Lock:Stupid'..msg.chat_id_) then
local username = text:match('^هينه @(.*)') or text:match('^هينها @(.*)') 
function EngineerSource(extra,result,success)
if result.id_ then  
if tonumber(result.id_) == tonumber(Engineer) then  
...
if DevAbs:sismember(Engineer.."Abs:AbsConstructor:"..msg.chat_id_,result.id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
...
if tonumber(result.sender_user_id_) == tonumber(Engineer) then 
Dev_Abs(msg.chat_id_, msg.id_, 1, 'شو تمضرط اكو واحد يهين نفسه؟🤔👌🏿', 1, 'md') 
return false  
end  
...
if DevAbs:sismember(Engineer.."Abs:AbsConstructor:"..msg.chat_id_,result.sender_user_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, 'دي لكك تريد اهينن تاج راسكك؟😏🖕🏿', 1, 'md')
return false
end 
local EngineerSource = "صارر ستاذيي 🏃🏻‍♂️♥️" 
...
if DevAbs:sismember(Engineer..'Abs:AbsSudo:',result.sender_user_id_) then
Abssudo = 'المطورين الاساسيين • ' else Abssudo = '' end
if DevAbs:sismember(Engineer..'Abs:SecondSudo:',result.sender_user_id_) then
secondsudo = 'المطورين الثانويين • ' else secondsudo = '' end
if DevAbs:sismember(Engineer..'Abs:SudoBot:',result.sender_user_id_) then
sudobot = 'المطورين • ' else sudobot = '' end
if DevAbs:sismember(Engineer..'Abs:BasicConstructor:'..msg.chat_id_, result.sender_user_id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevAbs:sismember(Engineer..'Abs:Constructor:'..msg.chat_id_, result.sender_user_id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(Engineer..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_) then
manager = 'المدراء • ' else manager = '' end
if DevAbs:sismember(Engineer..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevAbs:sismember(Engineer..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(Engineer..'Abs:Cleaner:'..msg.chat_id_, result.sender_user_id_) then
...
DevAbs:srem(Engineer..'Abs:AbsSudo:', result.sender_user_id_)
DevAbs:srem(Engineer..'Abs:SecondSudo:', result.sender_user_id_)
DevAbs:srem(Engineer..'Abs:SudoBot:', result.sender_user_id_)
DevAbs:srem(Engineer..'Abs:BasicConstructor:'..msg.chat_id_,result.sender_user_id_)
DevAbs:srem(Engineer..'Abs:Constructor:'..msg.chat_id_,result.sender_user_id_)
DevAbs:srem(Engineer..'Abs:Managers:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(Engineer..'Abs:Admins:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(Engineer..'Abs:VipMem:'..msg.chat_id_, result.sender_user_id_)
DevAbs:srem(Engineer..'Abs:Cleaner:'..msg.chat_id_, result.sender_user_id_)
...
if DevAbs:sismember(Engineer..'Abs:AbsSudo:',result.id_) then
Abssudo = 'المطورين الاساسيين • ' else Abssudo = '' end
if DevAbs:sismember(Engineer..'Abs:SecondSudo:',result.id_) then
secondsudo = 'المطورين الثانويين • ' else secondsudo = '' end
if DevAbs:sismember(Engineer..'Abs:SudoBot:',result.id_) then
sudobot = 'المطورين • ' else sudobot = '' end
if DevAbs:sismember(Engineer..'Abs:BasicConstructor:'..msg.chat_id_, result.id_) then
basicconstructor = 'المنشئين الاساسيين • ' else basicconstructor = '' end
if DevAbs:sismember(Engineer..'Abs:Constructor:'..msg.chat_id_, result.id_) then
constructor = 'المنشئين • ' else constructor = '' end 
if DevAbs:sismember(Engineer..'Abs:Managers:'..msg.chat_id_, result.id_) then
manager = 'المدراء • ' else manager = '' end
if DevAbs:sismember(Engineer..'Abs:Admins:'..msg.chat_id_, result.id_) then
admins = 'الادمنيه • ' else admins = '' end
if DevAbs:sismember(Engineer..'Abs:VipMem:'..msg.chat_id_, result.id_) then
vipmem = 'المميزين • ' else vipmem = '' end
if DevAbs:sismember(Engineer..'Abs:Cleaner:'..msg.chat_id_, result.id_) then
cleaner = 'المنظفين • ' else cleaner = ''
end
...
if AbsDelAll(msg.sender_user_id_,msg.chat_id_) == 'sudoid' then
DevAbs:srem(Engineer..'Abs:AbsSudo:', result.id_)
DevAbs:srem(Engineer..'Abs:SecondSudo:', result.id_)
DevAbs:srem(Engineer..'Abs:SudoBot:', result.id_)
DevAbs:srem(Engineer..'Abs:BasicConstructor:'..msg.chat_id_,result.id_)
DevAbs:srem(Engineer..'Abs:Constructor:'..msg.chat_id_,result.id_)
DevAbs:srem(Engineer..'Abs:Managers:'..msg.chat_id_, result.id_)
DevAbs:srem(Engineer..'Abs:Admins:'..msg.chat_id_, result.id_)
DevAbs:srem(Engineer..'Abs:VipMem:'..msg.chat_id_, result.id_)
DevAbs:srem(Engineer..'Abs:Cleaner:'..msg.chat_id_, result.id_)
elseif AbsDelAll(msg.sender_user_id_,msg.chat_id_) == 'Abssudo' then
DevAbs:srem(Engineer..'Abs:SecondSudo:', result.id_)
DevAbs:srem(Engineer..'Abs:SudoBot:', result.id_)
DevAbs:srem(Engineer..'Abs:BasicConstructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(Engineer..'Abs:AbsSudo:',result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(Engineer..'Abs:AbsSudo:',result.id_)
...
DevAbs:sadd(Engineer..'Abs:AbsSudo:',user)
...
DevAbs:srem(Engineer..'Abs:AbsSudo:',user)
...
function sudo_reply(extra, result, success)
DevAbs:sadd(Engineer..'Abs:SecondSudo:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(Engineer..'Abs:SecondSudo:',result.id_)
...
DevAbs:sadd(Engineer..'Abs:SecondSudo:',user)
...
DevAbs:srem(Engineer..'Abs:SecondSudo:',user)
...
function sudo_reply(extra, result, success)
DevAbs:sadd(Engineer..'Abs:SudoBot:',result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(Engineer..'Abs:SudoBot:',result.id_)
...
DevAbs:sadd(Engineer..'Abs:SudoBot:',user)
...
DevAbs:srem(Engineer..'Abs:SudoBot:',user)
...
DevAbs:sadd(Engineer..'Abs:AbsConstructor:'..msg.chat_id_,result.sender_user_id_)
...
function promreply(extra,result,success)
if result.id_ then
DevAbs:sadd(Engineer..'Abs:AbsConstructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(Engineer..'Abs:AbsConstructor:'..msg.chat_id_,user)
...
DevAbs:srem(Engineer..'Abs:AbsConstructor:'..msg.chat_id_,result.sender_user_id_)
...
DevAbs:srem(Engineer..'Abs:AbsConstructor:'..msg.chat_id_,result.id_)
...
DevAbs:srem(Engineer..'Abs:AbsConstructor:'..msg.chat_id_,user)
...
function raf_reply(extra, result, success)
if not AbsConstructor(msg) and DevAbs:get(Engineer.."Abs:Lock:ProSet"..msg.chat_id_) then 
Dev_Abs(msg.chat_id_, msg.id_, 1,'᥀︙لاتستطيع رفع احد وذالك بسبب تعطيل الرفع', 1, 'md')
return false
end
DevAbs:sadd(Engineer..'Abs:BasicConstructor:'..msg.chat_id_,result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(Engineer..'Abs:BasicConstructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(Engineer..'Abs:BasicConstructor:'..msg.chat_id_,user)
...
DevAbs:srem(Engineer..'Abs:BasicConstructor:'..msg.chat_id_,user)
...
DevAbs:sadd(Engineer..'Abs:Constructor:'..msg.chat_id_,result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(Engineer..'Abs:Constructor:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(Engineer..'Abs:Constructor:'..msg.chat_id_,user)
...
DevAbs:srem(Engineer..'Abs:Constructor:'..msg.chat_id_,user)
...
DevAbs:sadd(Engineer..'Abs:Managers:'..msg.chat_id_,user)
...
DevAbs:srem(Engineer..'Abs:Managers:'..msg.chat_id_,user)
...
DevAbs:sadd(Engineer..'Abs:Cleaner:'..msg.chat_id_,result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(Engineer..'Abs:Cleaner:'..msg.chat_id_,result.id_)
...
DevAbs:sadd(Engineer..'Abs:Cleaner:'..msg.chat_id_,user)
...
DevAbs:srem(Engineer..'Abs:Cleaner:'..msg.chat_id_,user)
...
DevAbs:sadd(Engineer..'Abs:Admins:'..msg.chat_id_,user)
...
DevAbs:srem(Engineer..'Abs:Admins:'..msg.chat_id_,user)
...
DevAbs:sadd(Engineer..'Abs:VipMem:'..msg.chat_id_,user)
...
DevAbs:srem(Engineer..'Abs:VipMem:'..msg.chat_id_,user)
...
function promote_by_reply(extra, result, success)
local Check = https.request('https://api.telegram.org/bot'..TokenBot..'/getChatMember?chat_id='..msg.chat_id_..'&user_id='..Engineer)
local GetInfo = JSON.decode(Check)
...
EngineerSource = {"ها هلاو","انطق","كول","تفضل","احبك","عمري","لاف"};
...
DevAbs:del(Engineer.."Abs:Lock:ProSet"..msg.chat_id_)
...
DevAbs:set(Engineer.."Abs:Lock:ProSet"..msg.chat_id_,"true")
...
if not Constructor(msg) and DevAbs:get(Engineer.."Abs:Lock:KickBan"..msg.chat_id_) then 
...
ChatKick(result.chat_id_, result.sender_user_id_)
DevAbs:sadd(Engineer..'Abs:Ban:'..msg.chat_id_, result.sender_user_id_)
...
ChatKick(msg.chat_id_, result.id_)
DevAbs:sadd(Engineer..'Abs:Ban:'..msg.chat_id_, result.id_)
...
ChatKick(msg.chat_id_, user)
DevAbs:sadd(Engineer..'Abs:Ban:'..msg.chat_id_, user)
...
DevAbs:srem(Engineer..'Abs:Ban:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
DevAbs:srem(Engineer..'Abs:Ban:'..msg.chat_id_, result.id_)
...
DevAbs:srem(Engineer..'Abs:Ban:'..msg.chat_id_, user)
...
if not Constructor(msg) and DevAbs:get(Engineer.."Abs:Lock:MuteTked"..msg.chat_id_) then 
...
if DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","᥀︙هو بالفعل مكتوم من المجموعه")  
else
DevAbs:sadd(Engineer..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_)
...
if DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_, result.id_) then
ReplyStatus(msg,result.id_,"Reply","᥀︙هو بالفعل مكتوم من المجموعه")  
else
DevAbs:sadd(Engineer..'Abs:Muted:'..msg.chat_id_, result.id_)
...
if DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_, user) then
ReplyStatus(msg,user,"Reply","᥀︙هو بالفعل مكتوم من المجموعه")  
else
DevAbs:sadd(Engineer..'Abs:Muted:'..msg.chat_id_, user)
...
if not DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","᥀︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(Engineer..'Abs:Muted:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
if not DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_, result.id_) then
ReplyStatus(msg,result.id_,"Reply","᥀︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(Engineer..'Abs:Muted:'..msg.chat_id_, result.id_)
...
if not DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_, user) then
ReplyStatus(msg,user,"Reply","᥀︙هو ليس مكتوم لالغاء كتمه")  
else
DevAbs:srem(Engineer..'Abs:Muted:'..msg.chat_id_, user)
...
DevAbs:sadd(Engineer..'Abs:Tkeed:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:sadd(Engineer..'Abs:Tkeed:'..msg.chat_id_, result.id_)
...
DevAbs:sadd(Engineer..'Abs:Tkeed:'..msg.chat_id_, user)
...
DevAbs:srem(Engineer..'Abs:Tkeed:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:srem(Engineer..'Abs:Tkeed:'..msg.chat_id_, result.id_)
...
DevAbs:srem(Engineer..'Abs:Tkeed:'..msg.chat_id_, user)
...
if DevAbs:sismember(Engineer..'Abs:AbsSudo:',result.sender_user_id_) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي*", 1, 'md')
return false 
end
if DevAbs:sismember(Engineer..'Abs:SecondSudo:',result.sender_user_id_) and not AbsSudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي²*", 1, 'md')
return false 
end
ChatKick(result.chat_id_, result.sender_user_id_)
DevAbs:sadd(Engineer..'Abs:BanAll:', result.sender_user_id_)
...
if DevAbs:sismember(Engineer..'Abs:AbsSudo:',result.id_) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي*", 1, 'md')
return false 
end
if DevAbs:sismember(Engineer..'Abs:SecondSudo:',result.id_) and not AbsSudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي²*", 1, 'md')
return false 
end
if result.id_ then
ChatKick(msg.chat_id_, result.id_)
DevAbs:sadd(Engineer..'Abs:BanAll:', result.id_)
...
if tonumber(user) == tonumber(Engineer) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر البوت عام*", 1, 'md')
return false 
end
...
if DevAbs:sismember(Engineer..'Abs:AbsSudo:',user) and not Sudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي*", 1, 'md')
return false 
end
if DevAbs:sismember(Engineer..'Abs:SecondSudo:',user) and not AbsSudo(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙*لاتستطيع حظر المطور الاساسي²*", 1, 'md')
return false 
end
ChatKick(msg.chat_id_, user)
DevAbs:sadd(Engineer..'Abs:BanAll:', user)
...
DevAbs:sadd(Engineer..'Abs:MuteAll:', result.sender_user_id_)
...
if result.id_ then
DevAbs:sadd(Engineer..'Abs:MuteAll:', result.id_)
...
DevAbs:sadd(Engineer..'Abs:MuteAll:', user)
...
DevAbs:srem(Engineer..'Abs:BanAll:', result.sender_user_id_)
DevAbs:srem(Engineer..'Abs:MuteAll:', result.sender_user_id_)
...
if result.id_ then
DevAbs:srem(Engineer..'Abs:BanAll:', result.id_)
DevAbs:srem(Engineer..'Abs:MuteAll:', result.id_)
...
DevAbs:srem(Engineer..'Abs:BanAll:', user)
DevAbs:srem(Engineer..'Abs:MuteAll:', user)
...
DevAbs:setex(Engineer.."Abs:EditDev"..msg.sender_user_id_,300,true)
end
if DevAbs:get(Engineer.."Abs:EditDev"..msg.sender_user_id_) then
if text and text:match("^الغاء$") then 
send(msg.chat_id_, msg.id_,'᥀︙تم الغاء امر تغير المطور الاساسي')
DevAbs:del(Engineer.."Abs:EditDev"..msg.sender_user_id_)
return false
end
if text and text:match("^(%d+)$") then 
...
if dp.first_name_ ~= false then
DevAbs:del(Engineer.."Abs:EditDev"..msg.sender_user_id_)
DevAbs:set(Engineer.."Abs:NewDev"..msg.sender_user_id_,dp.id_)
...
--     Source Engineer     --
if msg.reply_to_message_id_ ~= 0 then
if text and text:match("^رفع مطي$") and not DevAbs:get(Engineer..'Abs:Lock:Stupid'..msg.chat_id_) and ChCheck(msg) then
function donky_by_reply(extra, result, success)
if DevAbs:sismember(Engineer..'User:Donky:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","᥀︙هو مطي شرفع منه بعد😹💔") 
else
ReplyStatus(msg,result.sender_user_id_,"Reply","᥀︙تم رفعه في قائمة المطايه") 
DevAbs:sadd(Engineer..'User:Donky:'..msg.chat_id_, result.sender_user_id_)
end end
getMessage(msg.chat_id_, msg.reply_to_message_id_,donky_by_reply)
end end
--     Source Engineer     --
if msg.reply_to_message_id_ ~= 0  then
if text and text:match("^تنزيل مطي$") and not DevAbs:get(Engineer..'Abs:Lock:Stupid'..msg.chat_id_) and ChCheck(msg) then
function donky_by_reply(extra, result, success)
if not DevAbs:sismember(Engineer..'User:Donky:'..msg.chat_id_, result.sender_user_id_) then
ReplyStatus(msg,result.sender_user_id_,"Reply","᥀︙هو ليس مطي ليتم تنزيله") 
else
DevAbs:srem(Engineer..'User:Donky:'..msg.chat_id_, result.sender_user_id_)
...
DevAbs:set('EngineerSource:'..Engineer..'id:user'..msg.chat_id_,TXT)  
DevAbs:setex('EngineerSource:'..Engineer.."numadd:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
DevAbs:del(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..result.sender_user_id_) 
DevAbs:incrby(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..result.sender_user_id_,Num) 
...
DevAbs:set('EngineerSource:'..Engineer..'ids:user'..msg.chat_id_,TXT)  
DevAbs:setex('EngineerSource:'..Engineer.."nmadd:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)  
...
function Reply(extra, result, success)
DevAbs:incrby(Engineer..'Abs:GamesNumber'..msg.chat_id_..result.sender_user_id_,Num) 
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم اضافة "..Num..' نقطه', 1, 'md') 
end
tdcli_function ({ID = "GetMessage",chat_id_=msg.chat_id_,message_id_=tonumber(msg.reply_to_message_id_)},Reply, nil)
return false
end
if DevAbs:get(Engineer..'Abs:Lock:Clean'..msg.chat_id_) then if msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.photo_ or msg.content_.animation_ or msg.content_.animated_ then if msg.reply_to_message_id_ ~= 0 then DevAbs:sadd(Engineer.."Abs:cleaner"..msg.chat_id_, msg.id_) else DevAbs:sadd(Engineer.."Abs:cleaner"..msg.chat_id_, msg.id_) end end end
if DevAbs:get(Engineer..'Abs:Lock:CleanNum'..msg.chat_id_) then if msg.content_.video_ or msg.content_.document_ or msg.content_.sticker_ or msg.content_.photo_ or msg.content_.animation_ or msg.content_.animated_ then if msg.reply_to_message_id_ ~= 0 then DevAbs:sadd(Engineer.."Abs:cleanernum"..msg.chat_id_, msg.id_) else DevAbs:sadd(Engineer.."Abs:cleanernum"..msg.chat_id_, msg.id_) end end end
if Manager(msg) and msg.reply_to_message_id_ ~= 0 then
if text and text:match("^تثبيت$") and ChCheck(msg) then 
if DevAbs:sismember(Engineer.."Abs:Lock:Pinpin",msg.chat_id_) and not BasicConstructor(msg) then
...
if data.ID == "Ok" then
DevAbs:set(Engineer..'Abs:PinnedMsg'..msg.chat_id_,msg.reply_to_message_id_)
...
local List = DevAbs:smembers(Engineer..'Abs:VipMem:'..msg.chat_id_)
text = "᥀︙قائمة المميزين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Engineer..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Engineer..Abs)
text = "᥀︙قائمة الادمنيه ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Engineer..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Engineer..'Abs:Managers:'..msg.chat_id_)
text = "᥀︙قائمة المدراء ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Engineer..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Engineer..'Abs:Cleaner:'..msg.chat_id_)
text = "᥀︙قائمة المنظفين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Engineer..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Engineer..'Abs:Constructor:'..msg.chat_id_)
text = "᥀︙قائمة المنشئين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Engineer..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Engineer..'Abs:AbsConstructor:'..msg.chat_id_)
text = "᥀︙قائمة المالكين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Engineer..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Engineer..'Abs:BasicConstructor:'..msg.chat_id_)
...
local List = DevAbs:smembers(Engineer..'Abs:Muted:'..msg.chat_id_)
...
local List = DevAbs:smembers(Engineer..'Abs:Tkeed:'..msg.chat_id_)
text = "᥀︙قائمة المقيدين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local username = DevAbs:get(Engineer..'Save:UserName'..v)
if username then
...
local List = DevAbs:smembers(Engineer..'Abs:Ban:'..msg.chat_id_)
...
local List = DevAbs:hkeys(Engineer..'Abs:Filters:'..msg.chat_id_)
text = "᥀︙قائمة المنع ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k, v in pairs(List) do
text = text..k..'~ ❨ '..v..' ❩\n'
end
...
local List = DevAbs:smembers(Engineer..'User:Donky:'..msg.chat_id_)
...
local List = DevAbs:smembers(Engineer..'Abs:AbsSudo:')
...
local List = DevAbs:smembers(Engineer..'Abs:SecondSudo:')
...
local BanAll = DevAbs:smembers(Engineer..'Abs:BanAll:')
local MuteAll = DevAbs:smembers(Engineer..'Abs:MuteAll:')
if #BanAll ~= 0 then 
...
local List = DevAbs:smembers(Engineer..'Abs:SudoBot:')
text = "᥀︙قائمة المطورين ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n"
for k,v in pairs(List) do
local sudouser = DevAbs:get(Engineer..'Abs:Sudos'..v) 
local username = DevAbs:get(Engineer..'Save:UserName'..v)
if username then
...
if result.content_.sticker_ then
local idsticker = result.content_.sticker_.sticker_.persistent_id_
DevAbs:srem(Engineer.."Abs:FilterSteckr"..msg.chat_id_,idsticker)
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء منع الملصق يمكنهم ارساله الان', 1, 'md')
return false
end
if result.content_.ID == "MessagePhoto" then
local photo = result.content_.photo_.id_
DevAbs:srem(Engineer.."Abs:FilterPhoto"..msg.chat_id_,photo)
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء منع الصوره يمكنهم ارسالها الان', 1, 'md')
return false
end
if result.content_.animation_.animation_ then
local idanimation = result.content_.animation_.animation_.persistent_id_
DevAbs:srem(Engineer.."Abs:FilterAnimation"..msg.chat_id_,idanimation)
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Thwel:Abs'..msg.chat_id_) 
end
if text and (text == "تعطيل تحويل الصيغ" or text == "تعطيل التحويل") and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل تحويل الصيغ'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Thwel:Abs'..msg.chat_id_,true)  
end
if text == 'تحويل' and not DevAbs:get(Engineer..'Abs:Thwel:Abs'..msg.chat_id_) then  
if tonumber(msg.reply_to_message_id_) > 0 then 
...
tdcli_function ({ID = "GetUser",user_id_ = result.sender_user_id_},function(arg,data) 
local user_msgs = DevAbs:get(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..data.id_) or 0
local user_nkt = tonumber(DevAbs:get(Engineer..'Abs:GamesNumber'..msg.chat_id_..data.id_) or 0)
if DevAbs:sismember(Engineer..'Abs:BanAll:',result.sender_user_id_) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(Engineer..'Abs:MuteAll:',result.sender_user_id_) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(Engineer..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'محظور'
elseif DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(Engineer..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(Engineer..'Abs:SudoBot:',result.sender_user_id_) and SudoBot(msg) then
sudobot = '\n᥀︙عدد الكروبات ↫ '..(DevAbs:get(Engineer..'Abs:Sudos'..result.sender_user_id_) or 0)..'' 
else 
sudobot = '' 
end
...
local user_msgs = DevAbs:get(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..res.id_) or 0
local user_nkt = tonumber(DevAbs:get(Engineer..'Abs:GamesNumber'..msg.chat_id_..res.id_) or 0)
if DevAbs:sismember(Engineer..'Abs:BanAll:',res.id_) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(Engineer..'Abs:MuteAll:',res.id_) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(Engineer..'Abs:Ban:'..msg.chat_id_,res.id_) then
Tkeed = 'محظور'
elseif DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_,res.id_) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(Engineer..'Abs:Tkeed:'..msg.chat_id_,res.id_) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(Engineer..'Abs:SudoBot:',res.id_) and SudoBot(msg) then
sudobot = '\n᥀︙عدد الكروبات ↫ '..(DevAbs:get(Engineer..'Abs:Sudos'..res.id_) or 0)..'' 
else 
sudobot = '' 
end
...
local user_msgs = DevAbs:get(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..iduser) or 0
local user_nkt = tonumber(DevAbs:get(Engineer..'Abs:GamesNumber'..msg.chat_id_..iduser) or 0)
if DevAbs:sismember(Engineer..'Abs:BanAll:',iduser) then
Tkeed = 'محظور عام'
elseif DevAbs:sismember(Engineer..'Abs:MuteAll:',iduser) then
Tkeed = 'مكتوم عام'
elseif DevAbs:sismember(Engineer..'Abs:Ban:'..msg.chat_id_,iduser) then
Tkeed = 'محظور'
elseif DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_,iduser) then
Tkeed = 'مكتوم'
elseif DevAbs:sismember(Engineer..'Abs:Tkeed:'..msg.chat_id_,iduser) then
Tkeed = 'مقيد'
else
Tkeed = false
end
...
if DevAbs:sismember(Engineer..'Abs:SudoBot:',iduser) and SudoBot(msg) then
sudobot = '\n᥀︙عدد الكروبات ↫ '..(DevAbs:get(Engineer..'Abs:Sudos'..iduser) or 0)..'' 
else 
sudobot = '' 
end
...
if DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevAbs:sismember(Engineer..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) then banned = 'محظور' else banned = 'غير محظور' end
if DevAbs:sismember(Engineer..'Abs:BanAll:',result.sender_user_id_) then banall = 'محظور عام' else banall = 'غير محظور عام' end
if DevAbs:sismember(Engineer..'Abs:MuteAll:',result.sender_user_id_) then muteall = 'مكتوم عام' else muteall = 'غير مكتوم عام' end
if DevAbs:sismember(Engineer..'Abs:Tkeed:',result.sender_user_id_) then tkeed = 'مقيد' else tkeed = 'غير مقيد' end
...
if result.id_ then
if DevAbs:sismember(Engineer..'Abs:Muted:'..msg.chat_id_,result.id_) then muted = 'مكتوم' else muted = 'غير مكتوم' end
if DevAbs:sismember(Engineer..'Abs:Ban:'..msg.chat_id_,result.id_) then banned = 'محظور' else banned = 'غير محظور' end
if DevAbs:sismember(Engineer..'Abs:BanAll:',result.id_) then banall = 'محظور عام' else banall = 'غير محظور عام' end
if DevAbs:sismember(Engineer..'Abs:MuteAll:',result.id_) then muteall = 'مكتوم عام' else muteall = 'غير مكتوم عام' end
if DevAbs:sismember(Engineer..'Abs:Tkeed:',result.id_) then tkeed = 'مقيد' else tkeed = 'غير مقيد' end
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙الحظر العام ↫ '..banall..'\n᥀︙الكتم العام ↫ '..muteall..'\n᥀︙الحظر ↫ '..banned..'\n᥀︙الكتم ↫ '..muted..'\n᥀︙التقيد ↫ '..tkeed, 1, 'md')  
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙*المعرف غير صحيح*', 1, 'md')  
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.sender_user_id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Engineer..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(Engineer..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(Engineer..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(Engineer..'Abs:BanAll:',result.sender_user_id_) DevAbs:srem(Engineer..'Abs:MuteAll:',result.sender_user_id_)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.sender_user_id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Engineer..'Abs:Tkeed:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(Engineer..'Abs:Ban:'..msg.chat_id_,result.sender_user_id_) DevAbs:srem(Engineer..'Abs:Muted:'..msg.chat_id_,result.sender_user_id_) 
end
end
getMessage(msg.chat_id_, tonumber(msg.reply_to_message_id_),unbanreply) 
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..user.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Engineer..'Abs:Tkeed:'..msg.chat_id_,user) DevAbs:srem(Engineer..'Abs:Ban:'..msg.chat_id_,user) DevAbs:srem(Engineer..'Abs:Muted:'..msg.chat_id_,user) DevAbs:srem(Engineer..'Abs:BanAll:',user) DevAbs:srem(Engineer..'Abs:MuteAll:',user)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..user.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Engineer..'Abs:Tkeed:'..msg.chat_id_,user) DevAbs:srem(Engineer..'Abs:Ban:'..msg.chat_id_,user) DevAbs:srem(Engineer..'Abs:Muted:'..msg.chat_id_,user) 
end  
end,nil)  
end
...
if SecondSudo(msg) then
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Engineer..'Abs:Tkeed:'..msg.chat_id_,result.id_) DevAbs:srem(Engineer..'Abs:Ban:'..msg.chat_id_,result.id_) DevAbs:srem(Engineer..'Abs:Muted:'..msg.chat_id_,result.id_) DevAbs:srem(Engineer..'Abs:BanAll:',result.id_) DevAbs:srem(Engineer..'Abs:MuteAll:',result.id_)
else
HTTPS.request("https://api.telegram.org/bot"..TokenBot.."/restrictChatMember?chat_id="..msg.chat_id_.."&user_id=" ..result.id_.. "&can_send_messages=True&can_send_media_messages=True&can_send_other_messages=True&can_add_web_page_previews=True")  
DevAbs:srem(Engineer..'Abs:Tkeed:'..msg.chat_id_,result.id_) DevAbs:srem(Engineer..'Abs:Ban:'..msg.chat_id_,result.id_) DevAbs:srem(Engineer..'Abs:Muted:'..msg.chat_id_,result.id_) 
end
end,nil)   
end  
...
DevAbs:set(Engineer.."Abs:GpIds:Text"..msg.chat_id_,Text_Rand)
...
DevAbs:set("Engineer:New:id:"..Engineer..msg.sender_user_id_,'EngineerSource')
return "EngineerSource"
end
if text and DevAbs:get("Engineer:New:id:"..Engineer..msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ كليشة الايدي', 1, 'md')
DevAbs:del("Engineer:New:id:"..Engineer..msg.sender_user_id_)
return false
end
DevAbs:del("Engineer:New:id:"..Engineer..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم حفظ كليشة الايدي العامه', 1, 'md')
DevAbs:set(Engineer.."Abs:AllIds:Text",text)
return false
end
...
DevAbs:del(Engineer.."Abs:AllIds:Text")
end
end
--     Source Engineer     --
if text and text:match("^تعيين الايدي$") and ChCheck(msg) or text and text:match("^تعين الايدي$") and ChCheck(msg) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙رجائا اتبع التعليمات للتعيين \n᥀︙لطبع كليشة الايدي ارسل كليشه تحتوي على النصوص التي باللغه الانجليزيه ادناه ↫ ⤈\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n `#username` ↬ لطبع المعرف\n `#id` ↬ لطبع الايدي \n `#photos` ↬ لطبع عدد الصور \n `#stast` ↬ لطبع الرتب \n `#msgs` ↬ لطبع عدد الرسائل \n `#msgday` ↬ لطبع الرسائل اليوميه \n `#CustomTitle` ↬ لطبع اللقب \n `#bio` ↬ لطبع البايو \n `#auto` ↬ لطبع التفاعل \n `#game` ↬ لطبع عدد النقاط \n `#cont` ↬ لطبع عدد الجهات \n `#edit` ↬ لطبع عدد السحكات \n `#Description` ↬ لطبع تعليق الصور\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉', 1, 'md')
DevAbs:set("Engineer:New:id:"..Engineer..msg.chat_id_..msg.sender_user_id_,'EngineerSource')
return "EngineerSource"
end
if text and Manager(msg) and DevAbs:get("Engineer:New:id:"..Engineer..msg.chat_id_..msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ كليشة الايدي', 1, 'md')
DevAbs:del("Engineer:New:id:"..Engineer..msg.chat_id_..msg.sender_user_id_)
return false
end
DevAbs:del("Engineer:New:id:"..Engineer..msg.chat_id_..msg.sender_user_id_)
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم حفظ الكليشه الجديده', 1, 'md')
DevAbs:set(Engineer.."Abs:GpIds:Text"..msg.chat_id_,text)
return false
end
if text and text:match("^حذف الايدي$") and ChCheck(msg) or text and text:match("^مسح الايدي$") and ChCheck(msg) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم حذف كليشة الايدي")  
DevAbs:del(Engineer.."Abs:GpIds:Text"..msg.chat_id_)
end
end
--     Source Engineer     --
if msg.reply_to_message_id_ ~= 0 then
...
function EngineerSource(extra,Abs,success)
...
local msgsday = DevAbs:get(Engineer..'Abs:UsersMsgs'..Engineer..os.date('%d')..':'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local edit_msg = DevAbs:get(Engineer..'Abs:EditMsg'..msg.chat_id_..msg.sender_user_id_) or 0
local user_msgs = DevAbs:get(Engineer..'Abs:UsersMsgs'..msg.chat_id_..':'..msg.sender_user_id_)
local user_nkt = tonumber(DevAbs:get(Engineer..'Abs:GamesNumber'..msg.chat_id_..msg.sender_user_id_) or 0)
local cont = (tonumber(DevAbs:get(Engineer..'Abs:ContactNumber'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
...
if not DevAbs:get(Engineer..'Abs:Lock:Id'..msg.chat_id_) then 
if not DevAbs:get(Engineer..'Abs:Lock:Id:Photo'..msg.chat_id_) then 
if DevAbs:get(Engineer.."Abs:AllIds:Text") then
newpicid = DevAbs:get(Engineer.."Abs:AllIds:Text")
...
if not DevAbs:get(Engineer.."Abs:GpIds:Text"..msg.chat_id_) then 
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, Abs.photos_[0].sizes_[1].photo_.persistent_id_,newpicid,msg.id_,msg.id_.."")
else 
local new_id = DevAbs:get(Engineer.."Abs:GpIds:Text"..msg.chat_id_)
...
if DevAbs:get(Engineer.."Abs:AllIds:Text") then
newallid = DevAbs:get(Engineer.."Abs:AllIds:Text")
...
if DevAbs:get(Engineer.."Abs:AllIds:Text") then
notpicid = DevAbs:get(Engineer.."Abs:AllIds:Text")
...
if not DevAbs:get(Engineer..'Abs:Lock:EditMsgs'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم قفل التعديل")  
DevAbs:set(Engineer..'Abs:Lock:EditMsgs'..msg.chat_id_,true)
else
...
DevAbs:set(Engineer.."Abs:Lock:Bots"..msg.chat_id_,"del")  
end
if LockText[2] == "البوتات بالطرد" then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم قفل البوتات بالطرد")  
DevAbs:set(Engineer.."Abs:Lock:Bots"..msg.chat_id_,"kick")  
end
...
DevAbs:set(Engineer.."Abs:Lock:Bots"..msg.chat_id_,"ked")  
end
if LockText[2] == "التكرار" or LockText[2] == "التكرار بالحذف" then 
DevAbs:hset(Engineer.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","del")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم قفل التكرار بالحذف")  
end
if LockText[2] == "التكرار بالطرد" then 
DevAbs:hset(Engineer.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","kick")  
...
DevAbs:hset(Engineer.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم قفل التكرار بالتقيد")  
end
if LockText[2] == "التكرار بالكتم" then 
DevAbs:hset(Engineer.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","mute")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم قفل التكرار بالكتم")  
end
if BasicConstructor(msg) then
if LockText[2] == "التثبيت" then
if not DevAbs:get(Engineer..'Abs:Lock:Pin'..msg.chat_id_) then
tdcli_function ({ ID = "GetChannelFull",  channel_id_ = msg.chat_id_:gsub("-100","") }, function(arg,data)  DevAbs:set(Engineer.."Abs:PinnedMsg"..msg.chat_id_,data.pinned_message_id_)  end,nil)
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم قفل التثبيت")  
DevAbs:set(Engineer..'Abs:Lock:Pin'..msg.chat_id_,true)
DevAbs:sadd(Engineer.."Abs:Lock:Pinpin",msg.chat_id_) 
else
...
DevAbs:hset(Engineer.."Abs:Spam:Group:User"..msg.chat_id_ ,"Num:Spam" ,TextSpam) 
end
end
if text and (text:match("^ضع زمن التكرار (%d+)$") or text:match("^وضع زمن التكرار (%d+)$")) then  
local TextSpam = text:match("ضع زمن التكرار (%d+)$") or text:match("وضع زمن التكرار (%d+)$")
DevAbs:hset(Engineer.."Abs:Spam:Group:User"..msg.chat_id_ ,"Num:Spam:Time" ,TextSpam) 
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:Id:Photo'..msg.chat_id_)
end end
if text and text == 'تعطيل الايدي بالصوره' and ChCheck(msg) then
if DevAbs:get(Engineer..'Abs:Lock:Id:Photo'..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙الايدي بالصوره بالتاكيد معطل', 1, 'md')
else
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الايدي بالصوره'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Lock:Id:Photo'..msg.chat_id_,true)
end end 
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:Id'..msg.chat_id_)
end end 
if text and text == 'تعطيل الايدي' and ChCheck(msg) then
if DevAbs:get(Engineer..'Abs:Lock:Id'..msg.chat_id_) then
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙الايدي بالتاكيد معطل ', 1, 'md')
else
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الايدي بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Lock:Id'..msg.chat_id_,true)
end end
end
--     Source Engineer     --
...
DevAbs:setex(Engineer.."Abs:Set:Groups:Links"..msg.chat_id_..msg.sender_user_id_,300,true) 
end
if text == 'انشاء رابط' and ChCheck(msg) or text == 'انشاء الرابط' and ChCheck(msg) then
local LinkGp = json:decode(https.request('https://api.telegram.org/bot'..TokenBot..'/exportChatInviteLink?chat_id='..msg.chat_id_))
if not DevAbs:get(Engineer.."Abs:Groups:Links"..msg.chat_id_)  then 
if LinkGp.ok == true then 
LinkGroup = LinkGp.result
DevAbs:set(Engineer.."Abs:Groups:Links"..msg.chat_id_,LinkGroup) 
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer.."Abs:Lock:Welcome"..msg.chat_id_,true)
end
if text and text:match("^تعطيل الترحيب$") and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الترحيب بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer.."Abs:Lock:Welcome"..msg.chat_id_)
end
if DevAbs:get(Engineer..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_) then 
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ كليشة الترحيب', 1, 'md')
DevAbs:del(Engineer..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(Engineer..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_)
Welcomes = text:gsub('"',"") Welcomes = text:gsub("'","") Welcomes = text:gsub(",","") Welcomes = text:gsub("*","") Welcomes = text:gsub(";","") Welcomes = text:gsub("`","") Welcomes = text:gsub("{","") Welcomes = text:gsub("}","") 
DevAbs:set(Engineer..'Abs:Groups:Welcomes'..msg.chat_id_,Welcomes)
...
DevAbs:set(Engineer..'Abs:setwelcome'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
if text and text:match("^حذف الترحيب$") and ChCheck(msg) or text and text:match("^حذف ترحيب$") and ChCheck(msg) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم حذف الترحيب")  
DevAbs:del(Engineer..'Abs:Groups:Welcomes'..msg.chat_id_)
end
if text and text:match("^جلب الترحيب$") and ChCheck(msg) or text and text:match("^جلب ترحيب$") and ChCheck(msg) or text and text:match("^الترحيب$") and ChCheck(msg) then
local Welcomes = DevAbs:get(Engineer..'Abs:Groups:Welcomes'..msg.chat_id_)
if Welcomes then
Dev_Abs(msg.chat_id_, msg.id_, 1, Welcomes, 1, 'md')
else
...
--     Source Engineer     --
if DevAbs:get(Engineer..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_) then  
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء حفظ الوصف", 1, 'md')
DevAbs:del(Engineer..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(Engineer..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_)
...
DevAbs:set(Engineer..'Abs:SetDescription'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
DevAbs:set(Engineer.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_,"add")  
return false  
end    
if DevAbs:get(Engineer.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_) == "add" then
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر المنع', 1, 'md')
DevAbs:del(Engineer.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
return false  
end   
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم منع الكلمه ↫ "..text, 1, 'html')
DevAbs:del(Engineer.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
DevAbs:hset(Engineer..'Abs:Filters:'..msg.chat_id_, text,'newword')
return false
end
if text and text == "الغاء منع" and msg.reply_to_message_id_ == 0 and ChCheck(msg) then       
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙ارسل لي الكلمه الان", 1, 'md') 
DevAbs:set(Engineer.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_,"del")  
return false  
end    
if DevAbs:get(Engineer.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_) == "del" then   
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر الغاء المنع', 1, 'md')
DevAbs:del(Engineer.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
return false  
end   
if not DevAbs:hget(Engineer..'Abs:Filters:'..msg.chat_id_, text) then  
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙الكلمه ↫ "..text.." غير ممنوعه", 1, 'html')
DevAbs:del(Engineer.."Abs:SetFilters"..msg.sender_user_id_..msg.chat_id_)  
else
DevAbs:hdel(Engineer..'Abs:Filters:'..msg.chat_id_, text)
...
local users = DevAbs:scard(Engineer.."Abs:Users")
...
local gps = DevAbs:scard(Engineer.."Abs:Groups")
...
if not DevAbs:get(Engineer..'Delete:Time'..msg.chat_id_..':'..msg.sender_user_id_) then  
...
DevAbs:setex(Engineer..'Delete:Time'..msg.chat_id_..':'..msg.sender_user_id_,300,true)
end 
end
...
local pv = DevAbs:smembers(Engineer.."Abs:Users")
local sendok = 0
for i = 1, #pv do
...
DevAbs:srem(Engineer.."Abs:Users",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = DevAbs:smembers(Engineer.."Abs:Groups")
local w = 0
local q = 0
for i = 1, #group do
tdcli_function({ID='GetChat',chat_id_ = group[i]},function(arg,data)
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
DevAbs:srem(Engineer.."Abs:Groups",group[i]) 
tdcli_function ({ ID = "ChangeChatMemberStatus", chat_id_ = group[i], user_id_ = Engineer, status_ = { ID = "ChatMemberStatusLeft" }, }, dl_cb, nil)
w = w + 1
end
...
local Engineergp2 = (w + q)
local Engineergp3 = #group - Engineergp2
if q == 0 then
Engineergp2 = ''
else
...
Dev_Abs(msg.chat_id_, msg.id_, 1,'᥀︙عدد الكروبات الان ↫ { '..#group..' }'..Engineergp1..Engineergp2..'\n᥀︙العدد الحقيقي الان  ↫ ( '..Engineergp3..' ) \n ', 1, 'md')
end end
end,nil)
end
return false
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Lock:CleanNum'..msg.chat_id_,true)  
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:CleanNum'..msg.chat_id_) 
end
...
DevAbs:set(Engineer..'Abs:CleanNum'..msg.chat_id_,Num) 
end end 
if msg and DevAbs:get(Engineer..'Abs:Lock:CleanNum'..msg.chat_id_) then
if DevAbs:get(Engineer..'Abs:CleanNum'..msg.chat_id_) then CleanNum = DevAbs:get(Engineer..'Abs:CleanNum'..msg.chat_id_) else CleanNum = 200 end
if DevAbs:scard(Engineer.."Abs:cleanernum"..msg.chat_id_) >= tonumber(CleanNum) then 
local List = DevAbs:smembers(Engineer.."Abs:cleanernum"..msg.chat_id_)
local Del = 0
for k,v in pairs(List) do
Del = (Del + 1)
local Message = v
...
DevAbs:del(Engineer.."Abs:cleanernum"..msg.chat_id_)
end 
end
if CleanerNum(msg) then
if DevAbs:get(Engineer..'Abs:Lock:CleanNum'..msg.chat_id_) then 
if text == "التلقائي" and ChCheck(msg) or text == "عدد التلقائي" and ChCheck(msg) then 
local M = DevAbs:scard(Engineer.."Abs:cleanernum"..msg.chat_id_)
if M ~= 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙عدد الميديا ↫ "..M.."\n᥀︙الحذف التلقائي ↫ "..(DevAbs:get(Engineer..'Abs:CleanNum'..msg.chat_id_) or 200), 1, 'md') 
else
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل امسح بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Lock:Clean'..msg.chat_id_,true)  
end
if text == "تعطيل امسح" and Constructor(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل امسح بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:Clean'..msg.chat_id_) 
end
if Cleaner(msg) then
if DevAbs:get(Engineer..'Abs:Lock:Clean'..msg.chat_id_) then 
if text == "الميديا" and ChCheck(msg) or text == "عدد الميديا" and ChCheck(msg) then 
local M = DevAbs:scard(Engineer.."Abs:cleaner"..msg.chat_id_)
if M ~= 0 then
...
DevAbs:del(Engineer.."Abs:cleaner"..msg.chat_id_)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙لاتوجد ميديا هنا", 1, 'md') 
end end 
end
...
if DevAbs:get(Engineer..'Abs:Lock:EditMsgs'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم فتح التعديل")  
DevAbs:del(Engineer..'Abs:Lock:EditMsgs'..msg.chat_id_)
else
...
DevAbs:del(Engineer.."Abs:Lock:Bots"..msg.chat_id_)  
end
if UnLockText[2] == "التكرار" then 
DevAbs:hdel(Engineer.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User")  
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم فتح التكرار")  
end
if BasicConstructor(msg) then
if UnLockText[2] == "التثبيت" then
if DevAbs:get(Engineer..'Abs:Lock:Pin'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم فتح التثبيت")  
DevAbs:del(Engineer..'Abs:Lock:Pin'..msg.chat_id_)
DevAbs:srem(Engineer.."Abs:Lock:Pinpin",msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Text'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Photo'..msg.chat_id_)
else
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙الصور بالفعل مفتوحه في المجموعه', 1, 'md')
end
end
if UnLockText[2] == "الكلايش" then
if DevAbs:get(Engineer..'Abs:Lock:Spam'..msg.chat_id_) then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم فتح الكلايش")  
DevAbs:del(Engineer..'Abs:Lock:Spam'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Join'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Videos'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Document'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Inline'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Markdown'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Gifs'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Music'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Voice'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Links'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Location'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Tags'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Hashtak'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Contact'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:WebLinks'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Arabic'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:English'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:TagServr'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Stickers'..msg.chat_id_)
else
...
DevAbs:del(Engineer..'Abs:Lock:Forwards'..msg.chat_id_)
else
...
DevAbs:set(Engineer.."Abs:Lock:Bots"..msg.chat_id_,"del") DevAbs:hset(Engineer.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed") 
LockList ={'Abs:Lock:Links','Abs:Lock:Contact','Abs:Lock:Forwards','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:EditMsgs','Abs:Lock:Stickers','Abs:Lock:Farsi','Abs:Lock:Spam','Abs:Lock:WebLinks','Abs:Lock:Photo'}
for i,Lock in pairs(LockList) do
DevAbs:set(Engineer..Lock..msg.chat_id_,true)
end
...
DevAbs:del(Engineer..UnLock..msg.chat_id_)
end
...
DevAbs:del(Engineer..'Abs:Lock:Fshar'..msg.chat_id_) DevAbs:del(Engineer..'Abs:Lock:Taf'..msg.chat_id_) DevAbs:del(Engineer..'Abs:Lock:Kfr'..msg.chat_id_) 
DevAbs:set(Engineer.."Abs:Lock:Bots"..msg.chat_id_,"del") DevAbs:hset(Engineer.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User","keed") 
LockList ={'Abs:Lock:EditMsgs','Abs:Lock:Farsi','Abs:Lock:TagServr','Abs:Lock:Inline','Abs:Lock:Photo','Abs:Lock:Spam','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:Music','Abs:Lock:Voice','Abs:Lock:Links','Abs:Lock:Location','Abs:Lock:Tags','Abs:Lock:Stickers','Abs:Lock:Markdown','Abs:Lock:Forwards','Abs:Lock:Document','Abs:Lock:Contact','Abs:Lock:Hashtak','Abs:Lock:WebLinks'}
for i,Lock in pairs(LockList) do
DevAbs:set(Engineer..Lock..msg.chat_id_,true)
...
DevAbs:set(Engineer..'Abs:Lock:Fshar'..msg.chat_id_,true) DevAbs:set(Engineer..'Abs:Lock:Taf'..msg.chat_id_,true) DevAbs:set(Engineer..'Abs:Lock:Kfr'..msg.chat_id_,true) DevAbs:hdel(Engineer.."Abs:Spam:Group:User"..msg.chat_id_ ,"Spam:User") 
UnLockList ={'Abs:Lock:EditMsgs','Abs:Lock:Text','Abs:Lock:Arabic','Abs:Lock:English','Abs:Lock:Join','Abs:Lock:Bots','Abs:Lock:Farsi','Abs:Lock:FarsiBan','Abs:Lock:TagServr','Abs:Lock:Inline','Abs:Lock:Photo','Abs:Lock:Spam','Abs:Lock:Videos','Abs:Lock:Gifs','Abs:Lock:Music','Abs:Lock:Voice','Abs:Lock:Links','Abs:Lock:Location','Abs:Lock:Tags','Abs:Lock:Stickers','Abs:Lock:Markdown','Abs:Lock:Forwards','Abs:Lock:Document','Abs:Lock:Contact','Abs:Lock:Hashtak','Abs:Lock:WebLinks'}
for i,UnLock in pairs(UnLockList) do
DevAbs:del(Engineer..UnLock..msg.chat_id_)
end
...
DevAbs:set(Engineer..'Abs:Spam:Text'..msg.chat_id_,SetSpam)
...
DevAbs:set(Engineer.."Abs:SudoBot:Rd"..msg.chat_id_,Text)
...
DevAbs:set(Engineer.."Abs:BasicConstructor:Rd"..msg.chat_id_,Text)
...
DevAbs:set(Engineer.."Abs:Constructor:Rd"..msg.chat_id_,Text)
...
DevAbs:set(Engineer.."Abs:Managers:Rd"..msg.chat_id_,Text) 
...
DevAbs:set(Engineer.."Abs:Admins:Rd"..msg.chat_id_,Text)
...
DevAbs:set(Engineer.."Abs:VipMem:Rd"..msg.chat_id_,Text)
...
DevAbs:set(Engineer.."Abs:Cleaner:Rd"..msg.chat_id_,Text)
...
DevAbs:set(Engineer.."Abs:mem:Rd"..msg.chat_id_,Text)
...
DevAbs:del(Engineer.."Abs:mem:Rd"..msg.chat_id_)
DevAbs:del(Engineer.."Abs:Cleaner:Rd"..msg.chat_id_)
DevAbs:del(Engineer.."Abs:VipMem:Rd"..msg.chat_id_)
DevAbs:del(Engineer.."Abs:Admins:Rd"..msg.chat_id_)
DevAbs:del(Engineer.."Abs:Managers:Rd"..msg.chat_id_)
DevAbs:del(Engineer.."Abs:Constructor:Rd"..msg.chat_id_)
DevAbs:del(Engineer.."Abs:BasicConstructor:Rd"..msg.chat_id_)
DevAbs:del(Engineer.."Abs:SudoBot:Rd"..msg.chat_id_)
end
end
--     Source Engineer     --
...
if tonumber(admins[i].user_id_) ~= tonumber(Engineer) then
...
DevAbs:del(Engineer..'Abs:AbsSudo:')
...
DevAbs:del(Engineer..'Abs:BasicConstructor:'..msg.chat_id_)
end
end
if BasicConstructor(msg) then
if txts[2] == 'المنشئين' or txtss[2] == 'المنشئين' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم حذف المنشئين")  
DevAbs:del(Engineer..'Abs:Constructor:'..msg.chat_id_)
end end
if Constructor(msg) then
if txts[2] == 'المدراء' or txtss[2] == 'المدراء' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم حذف المدراء")  
DevAbs:del(Engineer..'Abs:Managers:'..msg.chat_id_)
end 
...
DevAbs:srem(Engineer..'Abs:Tkeed:'..msg.chat_id_, v)
end 
...
DevAbs:del(Engineer..'Abs:Filters:'..msg.chat_id_)
end
if txts[2] == 'قوائم المنع' or txtss[2] == 'قوائم المنع' then
ReplyStatus(msg,msg.sender_user_id_,"ReplyBy","᥀︙تم حذف قوائم المنع")  
DevAbs:del(Engineer..'Abs:Filters:'..msg.chat_id_)
DevAbs:del(Engineer.."Abs:FilterAnimation"..msg.chat_id_)
DevAbs:del(Engineer.."Abs:FilterPhoto"..msg.chat_id_)
DevAbs:del(Engineer.."Abs:FilterSteckr"..msg.chat_id_)
end
...
DevAbs:del(Engineer..'Abs:Ban:'..msg.chat_id_) DevAbs:del(Engineer..'Abs:Admins:'..msg.chat_id_) DevAbs:del(Engineer..'User:Donky:'..msg.chat_id_) DevAbs:del(Engineer..'Abs:VipMem:'..msg.chat_id_) DevAbs:del(Engineer..'Abs:Filters:'..msg.chat_id_) DevAbs:del(Engineer..'Abs:Muted:'..msg.chat_id_)
...
local basicconstructor = DevAbs:smembers(Engineer..'Abs:BasicConstructor:'..msg.chat_id_)
local constructor = DevAbs:smembers(Engineer..'Abs:Constructor:'..msg.chat_id_)
local Managers = DevAbs:smembers(Engineer..'Abs:Managers:'..msg.chat_id_)
local admins = DevAbs:smembers(Engineer..'Abs:Admins:'..msg.chat_id_)
local vipmem = DevAbs:smembers(Engineer..'Abs:VipMem:'..msg.chat_id_)
local donky = DevAbs:smembers(Engineer..'User:Donky:'..msg.chat_id_)
...
spam_c = DevAbs:get(Engineer..'Abs:Spam:Text'..msg.chat_id_)
end
--     Source Engineer     --
if DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_, "Spam:User") == "kick" then     
flood = "بالطرد"     
elseif DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "keed" then     
flood = "بالتقيد"     
elseif DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "mute" then     
flood = "بالكتم"           
elseif DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Spam:User") == "del" then     
flood = "بالحذف"
else     
flood = "مفتوح"     
end
--     Source Engineer     --
if DevAbs:get(Engineer.."Abs:Lock:Bots"..msg.chat_id_) == "del" then
lock_bots = "بالحذف"
elseif DevAbs:get(Engineer.."Abs:Lock:Bots"..msg.chat_id_) == "ked" then
lock_bots = "بالتقيد"   
elseif DevAbs:get(Engineer.."Abs:Lock:Bots"..msg.chat_id_) == "kick" then
lock_bots = "بالطرد"    
else
lock_bots = "مفتوحه"    
end
--     Source Engineer     --
if DevAbs:get(Engineer..'Abs:Lock:Text'..msg.chat_id_) then mute_text = 'مقفله' else mute_text = 'مفتوحه'end
if DevAbs:get(Engineer..'Abs:Lock:Photo'..msg.chat_id_) then mute_photo = 'مقفله' else mute_photo = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Videos'..msg.chat_id_) then mute_video = 'مقفله' else mute_video = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Gifs'..msg.chat_id_) then mute_gifs = 'مقفله' else mute_gifs = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Music'..msg.chat_id_) then mute_music = 'مقفله' else mute_music = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Inline'..msg.chat_id_) then mute_in = 'مقفله' else mute_in = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Voice'..msg.chat_id_) then mute_voice = 'مقفله' else mute_voice = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:EditMsgs'..msg.chat_id_) then mute_edit = 'مقفله' else mute_edit = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Links'..msg.chat_id_) then mute_links = 'مقفله' else mute_links = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Pin'..msg.chat_id_) then lock_pin = 'مقفله' else lock_pin = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Stickers'..msg.chat_id_) then lock_sticker = 'مقفله' else lock_sticker = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:TagServr'..msg.chat_id_) then lock_tgservice = 'مقفله' else lock_tgservice = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:WebLinks'..msg.chat_id_) then lock_wp = 'مقفله' else lock_wp = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Hashtak'..msg.chat_id_) then lock_htag = 'مقفله' else lock_htag = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Tags'..msg.chat_id_) then lock_tag = 'مقفله' else lock_tag = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Location'..msg.chat_id_) then lock_location = 'مقفله' else lock_location = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Contact'..msg.chat_id_) then lock_contact = 'مقفله' else lock_contact = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:English'..msg.chat_id_) then lock_english = 'مقفله' else lock_english = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Arabic'..msg.chat_id_) then lock_arabic = 'مقفله' else lock_arabic = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Forwards'..msg.chat_id_) then lock_forward = 'مقفله' else lock_forward = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Document'..msg.chat_id_) then lock_file = 'مقفله' else lock_file = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Markdown'..msg.chat_id_) then markdown = 'مقفله' else markdown = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Spam'..msg.chat_id_) then lock_spam = 'مقفله' else lock_spam = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Join'..msg.chat_id_) then lock_Join = 'مقفل' else lock_Join = 'مفتوح' end
if DevAbs:get(Engineer.."Abs:Lock:Welcome"..msg.chat_id_) then send_welcome = 'مقفله' else send_welcome = 'مفتوحه' end
if DevAbs:get(Engineer..'Abs:Lock:Fshar'..msg.chat_id_) then lock_fshar = 'مفتوح' else lock_fshar = 'مقفل' end
if DevAbs:get(Engineer..'Abs:Lock:Kfr'..msg.chat_id_) then lock_kaf = 'مفتوح' else lock_kaf = 'مقفل' end
if DevAbs:get(Engineer..'Abs:Lock:Taf'..msg.chat_id_) then lock_taf = 'مفتوحه' else lock_taf = 'مقفله' end
if DevAbs:get(Engineer..'Abs:Lock:Farsi'..msg.chat_id_) then lock_farsi = 'مقفله' else lock_farsi = 'مفتوحه' end
local Flood_Num = DevAbs:hget(Engineer.."Abs:Spam:Group:User"..msg.chat_id_,"Num:Spam") or 5
--     Source Engineer     --
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل ميزة انطق'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Antk:Abs'..msg.chat_id_) 
end
if text == "تعطيل انطق" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل ميزة انطق'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Antk:Abs'..msg.chat_id_,true)  
end
if text and text:match("^انطق (.*)$") and not DevAbs:get(Engineer..'Abs:Antk:Abs'..msg.chat_id_) and ChCheck(msg) then
...
--     Source Engineer     --
if DevAbs:get(Engineer..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_) then 
if text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء حفظ قوانين المجموعه', 1, 'md')
DevAbs:del(Engineer..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_)
return false  
end 
DevAbs:del(Engineer..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_)
DevAbs:set(Engineer..'Abs:rules'..msg.chat_id_,text)
...
DevAbs:set(Engineer..'Abs:setrules'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
end
if text and text:match("^القوانين$") and ChCheck(msg) then
local rules = DevAbs:get(Engineer..'Abs:rules'..msg.chat_id_)
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Zrf:Abs'..msg.chat_id_) 
end
if text == "تعطيل الزخرفه" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الزخرفه بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Zrf:Abs'..msg.chat_id_,true)  
end
if DevAbs:get(Engineer..'Zrf:Abs'..msg.chat_id_..msg.sender_user_id_) then 
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر الزخرفه', 1, 'md')
DevAbs:del(Engineer..'Zrf:Abs'..msg.chat_id_..msg.sender_user_id_)
return false  
end 
...
if not DevAbs:get(Engineer..'Abs:Zrf:Abs'..msg.chat_id_) then
if text == 'زخرفه' and ChCheck(msg) or text == 'الزخرفه' and ChCheck(msg) then  
DevAbs:setex(Engineer.."Zrf:Abs"..msg.chat_id_..msg.sender_user_id_,300,true)
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Brg:Abs'..msg.chat_id_) 
end
if text == "تعطيل الابراج" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الابراج بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Brg:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(Engineer..'Abs:Brg:Abs'..msg.chat_id_) then
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Nsba:Abs'..msg.chat_id_) 
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Nsba:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(Engineer..'Abs:Nsba:Abs'..msg.chat_id_) then
if text == "نسبه الحب" and ChCheck(msg) or text == "نسبة الحب" and ChCheck(msg) then
DevAbs:set(Engineer..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسمين لحساب نسبة الحب بينهما كمثال ↫ جاك و روز', 1, 'md')
end
end
if text and text ~= "نسبه الحب" and text ~= "نسبة الحب" and DevAbs:get(Engineer..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الحب ', 1, 'md')
DevAbs:del(Engineer..'LoveNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
...
DevAbs:set(Engineer..'RyNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true)
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسمين لحساب نسبة الخيانه بينهما كمثال ↫ جاك و روز', 1, 'md')
end
end
if text and text ~= "نسبه الخيانه" and text ~= "نسبة الخيانه" and text ~= "↫ نسبه الخيانه ᥀" and DevAbs:get(Engineer..'RyNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الخيانه ', 1, 'md')
DevAbs:del(Engineer..'RyNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(Engineer..'JNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسم الشخص لقياس نسبة جماله كمثال ↫ جاك او روز', 1, 'md')
end
end
if text and text ~= "نسبه الجمال" and text ~= "نسبة الجمال" and text ~= "↫ نسبه الجمال ᥀" and DevAbs:get(Engineer..'JNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الجمال ', 1, 'md')
DevAbs:del(Engineer..'JNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(Engineer..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسمين لحساب نسبة الكره بينهما كمثال ↫ جاك و روز', 1, 'md')
end
end
if text and text ~= "نسبه الكره" and text ~= "نسبة الكره" and text ~= "↫ نسبه الكره ᥀" and DevAbs:get(Engineer..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الكره ', 1, 'md')
DevAbs:del(Engineer..'HataNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(Engineer..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسم الشخص لقياس نسبة رجولته كمثال ↫ جاك', 1, 'md')
end
end
if text and text ~= "نسبه الرجوله" and text ~= "نسبة الرجوله" and text ~= "نسبه رجوله" and text ~= "نسبة رجوله" and text ~= "↫ نسبه الرجوله ᥀" and DevAbs:get(Engineer..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الرجوله ', 1, 'md')
DevAbs:del(Engineer..'RjolaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(Engineer..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسم الشخص لقياس نسبة انوثته كمثال ↫ روز', 1, 'md')
end
end
if text and text ~= "نسبه الانوثه" and text ~= "نسبة الانوثه" and text ~= "نسبه انوثه" and text ~= "نسبة انوثه" and text ~= "↫ نسبه الانوثه ᥀" and DevAbs:get(Engineer..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الانوثه ', 1, 'md')
DevAbs:del(Engineer..'AnothaNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
DevAbs:set(Engineer..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_,true) 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙قم بارسل اسم الشخص لقياس نسبة غبائه كمثال ↫ جاك او روز', 1, 'md')
end
end
if text and text ~= "نسبه الغباء" and text ~= "نسبة الغباء" and text ~= "↫ نسبه الغباء ᥀" and DevAbs:get(Engineer..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_) then
if text and text == 'الغاء' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, '᥀︙تم الغاء امر نسبة الغباء ', 1, 'md')
DevAbs:del(Engineer..'StupidNsba:Abs'..msg.chat_id_..msg.sender_user_id_) 
return false 
end 
Abs = math.random(0,100);
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل حساب العمر'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Age:Abs'..msg.chat_id_) 
end
if text == "تعطيل حساب العمر" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل حساب العمر'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Age:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(Engineer..'Abs:Age:Abs'..msg.chat_id_) then
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Mean:Abs'..msg.chat_id_) 
end
if text == "تعطيل معاني الاسماء" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل معاني الاسماء'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Mean:Abs'..msg.chat_id_,true)  
end
if not DevAbs:get(Engineer..'Abs:Mean:Abs'..msg.chat_id_) then
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل المتحركه'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:gif:Abs'..msg.chat_id_) 
end
if text == "تعطيل متحركه" and Manager(msg) and ChCheck(msg) or text == "تعطيل المتحركه" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل المتحركه'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:gif:Abs'..msg.chat_id_,true)  
end
if text == "متحركه" or text == "↫ متحركه ᥀" and not DevAbs:get(Engineer..'Abs:gif:Abs'..msg.chat_id_) and ChCheck(msg) then
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل الميمز'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:memz:Abs'..msg.chat_id_) 
end
if text == "تعطيل ميمز" and Manager(msg) and ChCheck(msg) or text == "تعطيل الميمز" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الميمز'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:memz:Abs'..msg.chat_id_,true)  
end
if text == "ميمز" or text == "↫ ميمز ᥀" and not DevAbs:get(Engineer..'Abs:memz:Abs'..msg.chat_id_) and ChCheck(msg) then
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل غنيلي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Audios:Abs'..msg.chat_id_) 
end
if text == "تعطيل غنيلي" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل غنيلي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Audios:Abs'..msg.chat_id_,true)  
end
if text == "غنيلي" or text == "↫ غنيلي ᥀" and not DevAbs:get(Engineer..'Abs:Audios:Abs'..msg.chat_id_) and ChCheck(msg) then
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل الاغاني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:mp3:Abs'..msg.chat_id_) 
end
if text == "تعطيل الاغاني" and Manager(msg) and ChCheck(msg) or text == "تعطيل اغنيه" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الاغاني'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:mp3:Abs'..msg.chat_id_,true)  
end
if text == "اغنيه" or text == "↫ اغنيه ᥀" or text == "اغاني" and not DevAbs:get(Engineer..'Abs:mp3:Abs'..msg.chat_id_) and ChCheck(msg) then
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل الريمكس'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Remix:Abs'..msg.chat_id_) 
end
if text == "تعطيل ريمكس" and Manager(msg) and ChCheck(msg) or text == "تعطيل الريمكس" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الريمكس'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Remix:Abs'..msg.chat_id_,true)  
end
if text == "ريمكس" or text == "↫ ريمكس ᥀" and not DevAbs:get(Engineer..'Abs:Remix:Abs'..msg.chat_id_) and ChCheck(msg) then
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل الصوره'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Photo:Abs'..msg.chat_id_) 
end
if text == "تعطيل صوره" and Manager(msg) and ChCheck(msg) or text == "تعطيل الصوره" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الصوره'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Photo:Abs'..msg.chat_id_,true)  
end
if text == "صوره" or text == "↫ صوره ᥀" and not DevAbs:get(Engineer..'Abs:Photo:Abs'..msg.chat_id_) and ChCheck(msg) then
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل الانمي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Anime:Abs'..msg.chat_id_) 
end
if text == "تعطيل انمي" and Manager(msg) and ChCheck(msg) or text == "تعطيل الانمي" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الانمي'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Anime:Abs'..msg.chat_id_,true)  
end
if text == "انمي" or text == "↫ انمي ᥀" and not DevAbs:get(Engineer..'Abs:Anime:Abs'..msg.chat_id_) and ChCheck(msg) then
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل الافلام'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Movies:Abs'..msg.chat_id_) 
end
if text == "تعطيل فلم" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الافلام'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Movies:Abs'..msg.chat_id_,true)  
end
if text == "فلم" or text == "↫ فلم ᥀" and not DevAbs:get(Engineer..'Abs:Movies:Abs'..msg.chat_id_) and ChCheck(msg) then
...
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل المسلسلات'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Series:Abs'..msg.chat_id_) 
end
if text == "تعطيل مسلسل" and Manager(msg) and ChCheck(msg) then
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل المسلسلات'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Series:Abs'..msg.chat_id_,true)  
end
if text == "مسلسل" or text == "↫ مسلسل ᥀" and not DevAbs:get(Engineer..'Abs:Series:Abs'..msg.chat_id_) and ChCheck(msg) then
...
--     Source Engineer     --
if Admin(msg) then
if DevAbs:get(Engineer..'Abs:LockSettings'..msg.chat_id_) then 
if text == "الروابط" then if DevAbs:get(Engineer..'Abs:Lock:Links'..msg.chat_id_) then mute_links = 'مقفله' else mute_links = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙الروابط ↫ "..mute_links.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "المعرف" or text == "المعرفات" then if DevAbs:get(Engineer..'Abs:Lock:Tags'..msg.chat_id_) then lock_tag = 'مقفوله' else lock_tag = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙المعرف ↫ "..lock_tag.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "المتحركه" or text == "الملصقات المتحركه" then if DevAbs:get(Engineer..'Abs:Lock:Gifs'..msg.chat_id_) then mute_gifs = 'مقفوله' else mute_gifs = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙المتحركه ↫ "..mute_gifs.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الملصقات" then if DevAbs:get(Engineer..'Abs:Lock:Stickers'..msg.chat_id_) then lock_sticker = 'مقفوله' else lock_sticker = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙الملصقات ↫ "..lock_sticker.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الصور" then if DevAbs:get(Engineer..'Abs:Lock:Photo'..msg.chat_id_) then mute_photo = 'مقفوله' else mute_photo = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙الصور ↫ "..mute_photo.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الفيديو" or text == "الفيديوهات" then if DevAbs:get(Engineer..'Abs:Lock:Videos'..msg.chat_id_) then mute_video = 'مقفوله' else mute_video = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙الفيديو ↫ "..mute_video.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الاونلاين" then if DevAbs:get(Engineer..'Abs:Lock:Inline'..msg.chat_id_) then mute_in = 'مقفل' else mute_in = 'مفتوح' end local EngineerSource = "\n" .."᥀︙الاونلاين ↫ "..mute_in.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الدردشه" then if DevAbs:get(Engineer..'Abs:Lock:Text'..msg.chat_id_) then mute_text = 'مقفله' else mute_text = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙الدردشه ↫ "..mute_text.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "التوجيه" or text == "اعاده التوجيه" then if DevAbs:get(Engineer..'Abs:Lock:Forwards'..msg.chat_id_) then lock_forward = 'مقفل' else lock_forward = 'مفتوح' end local EngineerSource = "\n" .."᥀︙التوجيه ↫ "..lock_forward.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الاغاني" then if DevAbs:get(Engineer..'Abs:Lock:Music'..msg.chat_id_) then mute_music = 'مقفوله' else mute_music = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙الاغاني ↫ "..mute_music.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الصوت" or text == "الصوتيات" then if DevAbs:get(Engineer..'Abs:Lock:Voice'..msg.chat_id_) then mute_voice = 'مقفول' else mute_voice = 'مفتوح' end local EngineerSource = "\n" .."᥀︙الصوت ↫ "..mute_voice.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الجهات" or text == "جهات الاتصال" then if DevAbs:get(Engineer..'Abs:Lock:Contact'..msg.chat_id_) then lock_contact = 'مقفوله' else lock_contact = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙الجهات ↫ "..lock_contact.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الماركداون" then if DevAbs:get(Engineer..'Abs:Lock:Markdown'..msg.chat_id_) then markdown = 'مقفل' else markdown = 'مفتوح' end local EngineerSource = "\n" .."᥀︙الماركداون ↫ "..markdown.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الهاشتاك" then if DevAbs:get(Engineer..'Abs:Lock:Hashtak'..msg.chat_id_) then lock_htag = 'مقفل' else lock_htag = 'مفتوح' end local EngineerSource = "\n" .."᥀︙الهاشتاك ↫ "..lock_htag.."\n"Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "التعديل" then if DevAbs:get(Engineer..'Abs:Lock:EditMsgs'..msg.chat_id_) then mute_edit = 'مقفل' else mute_edit = 'مفتوح' end local EngineerSource = "\n" .."᥀︙التعديل ↫ "..mute_edit.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "التثبيت" then if DevAbs:get(Engineer..'Abs:Lock:Pin'..msg.chat_id_) then lock_pin = 'مقفل' else lock_pin = 'مفتوح' end local EngineerSource = "\n" .."᥀︙التثبيت ↫ "..lock_pin.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الاشعارات" then if DevAbs:get(Engineer..'Abs:Lock:TagServr'..msg.chat_id_) then lock_tgservice = 'مقفوله' else lock_tgservice = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙الاشعارات ↫ "..lock_tgservice.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الكلايش" then if DevAbs:get(Engineer..'Abs:Lock:Spam'..msg.chat_id_) then lock_spam = 'مقفوله' else lock_spam = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙الكلايش ↫ "..lock_spam.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الدخول" then if DevAbs:get(Engineer..'Abs:Lock:Join'..msg.chat_id_) then lock_Join = 'مقفول' else lock_Join = 'مفتوح' end local EngineerSource = "\n" .."᥀︙الدخول ↫ "..lock_Join.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الشبكات" then if DevAbs:get(Engineer..'Abs:Lock:WebLinks'..msg.chat_id_) then lock_wp = 'مقفوله' else lock_wp = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙الشبكات ↫ "..lock_wp.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "المواقع" then if DevAbs:get(Engineer..'Abs:Lock:Location'..msg.chat_id_) then lock_location = 'مقفوله' else lock_location = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙المواقع ↫ "..lock_location.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "العربيه" then if DevAbs:get(Engineer..'Abs:Lock:Arabic'..msg.chat_id_) then lock_arabic = 'مقفوله' else lock_arabic = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙العربيه ↫ "..lock_arabic.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الانكليزيه" then if DevAbs:get(Engineer..'Abs:Lock:English'..msg.chat_id_) then lock_english = 'مقفوله' else lock_english = 'مفتوحه' end local EngineerSource = "\n" .."᥀︙الانكليزيه ↫ "..lock_english.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الكفر" then if DevAbs:get(Engineer..'Abs:Lock:Kfr'..msg.chat_id_) then lock_kaf = 'مفتوح' else lock_kaf = 'مقفل' end local EngineerSource = "\n" .."᥀︙الكفر ↫ "..lock_kaf.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الفشار" then if DevAbs:get(Engineer..'Abs:Lock:Fshar'..msg.chat_id_) then lock_fshar = 'مفتوح' else lock_fshar = 'مقفل' end local EngineerSource = "\n" .."᥀︙الفشار ↫ "..lock_fshar.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
if text == "الطائفيه" then if DevAbs:get(Engineer..'Abs:Lock:Taf'..msg.chat_id_) then lock_taf = 'مفتوحه' else lock_taf = 'مقفله' end local EngineerSource = "\n" .."᥀︙الطائفيه ↫ "..lock_taf.."\n" Dev_Abs(msg.chat_id_, msg.id_, 1, EngineerSource, 1, 'md') end
end
--     Source Engineer     --
if text == 'تفعيل كشف الاعدادات' and ChCheck(msg) then 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل كشف الاعدادات'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:LockSettings'..msg.chat_id_,true)  
end
if text == 'تعطيل كشف الاعدادات' and ChCheck(msg) then 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل كشف الاعدادات'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:LockSettings'..msg.chat_id_) 
end
--     Source Engineer     --
if text == 'تفعيل اوامر التحشيش' and Manager(msg) and ChCheck(msg) then 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل اوامر التحشيش'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:Stupid'..msg.chat_id_)
end
if text == 'تعطيل اوامر التحشيش' and Manager(msg) and ChCheck(msg) then 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل اوامر التحشيش'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Lock:Stupid'..msg.chat_id_,true)
end
--     Source Engineer     --
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:Robot'..msg.chat_id_)
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Lock:Robot'..msg.chat_id_,true)
end
--     Source Engineer     --
if text == 'تفعيل ردود المدير' and Manager(msg) and ChCheck(msg) then 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل ردود المدير'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:GpRed'..msg.chat_id_)
end
if text == 'تعطيل ردود المدير' and Manager(msg) and ChCheck(msg) then 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل ردود المدير'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Lock:GpRed'..msg.chat_id_,true)
end
--     Source Engineer     --
if text == 'تفعيل ردود المطور' and Manager(msg) and ChCheck(msg) then 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل ردود المطور'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:AllRed'..msg.chat_id_)
end
if text == 'تعطيل ردود المطور' and Manager(msg) and ChCheck(msg) then 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل ردود المطور'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Lock:AllRed'..msg.chat_id_,true)
end
--     Source Engineer     --
if SecondSudo(msg) then
...
--     Source Engineer     --
if msg.content_.photo_ then
if DevAbs:get(Engineer..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_) then
if msg.content_.photo_.sizes_[3] then
...
DevAbs:del(Engineer..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_)
return false  end
if data.message_ == "CHAT_ADMIN_REQUIRED" then 
send(msg.chat_id_, msg.id_,"᥀︙ليست لدي صلاحية تغير معلومات المجموعه يرجى التحقق من الصلاحيات") 
DevAbs:del(Engineer..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_)
...
DevAbs:set(Engineer..'Abs:SetPhoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
end
--     Source Engineer     --
...
if data.ID == "Ok" then
DevAbs:del(Engineer..'Abs:PinnedMsg'..msg.chat_id_)
...
local PinId = DevAbs:get(Engineer..'Abs:PinnedMsg'..msg.chat_id_)
if PinId then
...
DevAbs:del(Engineer..'Abs:Ban:'..msg.chat_id_)
else
local x = 0
...
local Muted = DevAbs:scard(Engineer.."Abs:Muted:"..msg.chat_id_) or "0"
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,dp) 
tdcli_function({ID="GetChannelFull",channel_id_ = msg.chat_id_:gsub("-100","")},function(arg,data) 
...
local ConstructorList = DevAbs:scard(Engineer.."Abs:Constructor:"..ChatId) or 0
local BanedList = DevAbs:scard(Engineer.."Abs:Ban:"..ChatId) or 0
local ManagerList = DevAbs:scard(Engineer.."Abs:Managers:"..ChatId) or 0
local MutedList = DevAbs:scard(Engineer.."Abs:Muted:"..ChatId) or 0
local TkeedList = DevAbs:scard(Engineer.."Abs:Abs:Tkeed:"..ChatId) or 0
...
ChatLeave(dp.id_, Engineer)
DevAbs:srem(Engineer.."Abs:Groups", dp.id_)
else
...
DevAbs:set(Engineer..'Abs:Num:Add:Bot',Num) 
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:FreeBot'..Engineer) 
end 
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Lock:FreeBot'..Engineer,true) 
end 
end
if ChatType == 'sp' or ChatType == 'gp'  then
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Photo:Profile'..msg.chat_id_) 
end
if text == 'تفعيل صورتي' and Manager(msg) and ChCheck(msg) then  
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل صورتي بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Photo:Profile'..msg.chat_id_,true)  
end
if text == 'تفعيل الالعاب' and Manager(msg) and ChCheck(msg) or text == 'تفعيل اللعبه' and Manager(msg) and ChCheck(msg) then   
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تفعيل الالعاب بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:Games'..msg.chat_id_) 
end
if text == 'تعطيل الالعاب' and Manager(msg) and ChCheck(msg) or text == 'تعطيل اللعبه' and Manager(msg) and ChCheck(msg) then  
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم تعطيل الالعاب بنجاح'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer..'Abs:Lock:Games'..msg.chat_id_,true)  
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer..'Abs:Lock:Gamesinline'..msg.chat_id_) 
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer.."Abs:Lock:GpLinks"..msg.chat_id_,"ok")
return false  
end
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:del(Engineer.."Abs:Lock:Rd"..msg.chat_id_)
return false  
end
end
...
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
DevAbs:set(Engineer.."Abs:Lock:Rd"..msg.chat_id_,"ok")
return false  
end
end
...
ReplyStatus(msg,result.id_,"ReplyBy","᥀︙تم تفعيل المجموعه "..dp.title_)  
DevAbs:sadd(Engineer.."Abs:Groups",msg.chat_id_)
if not DevAbs:get(Engineer..'Abs:SudosGp'..msg.sender_user_id_..msg.chat_id_) and not SecondSudo(msg) then 
DevAbs:incrby(Engineer..'Abs:Sudos'..msg.sender_user_id_,1)
DevAbs:set(Engineer..'Abs:SudosGp'..msg.sender_user_id_..msg.chat_id_,"Abs")
end
local Name1 = result.first_name_
local Name1 = Name1:gsub('"',"") 
local Name1 = Name1:gsub("'","") 
...
tdcli_function ({ID = "GetUser",user_id_ = msg.sender_user_id_},function(extra,result,success)
tdcli_function({ID ="GetChat",chat_id_=msg.chat_id_},function(arg,dp) 
if not DevAbs:sismember(Engineer..'Abs:Groups',msg.chat_id_) then
...
for k,v in pairs(List) do
local GroupsManagers = DevAbs:scard(Engineer.."Abs:Managers:"..v) or 0
local GroupsAdmins = DevAbs:scard(Engineer.."Abs:Admins:"..v) or 0
local Groupslink = DevAbs:get(Engineer.."Abs:Groups:Links" ..v)
...
DevAbs:setex(Engineer.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n᥀︙للخروج ارسل ↫ ( الغاء ) \n ✓'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(Engineer.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(Engineer.."Abs:Send:Pv"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
List = DevAbs:smembers(Engineer..'Abs:Users') 
if msg.content_.text_ then
for k,v in pairs(List) do 
AbsText = "الرساله"
...
DevAbs:setex(Engineer.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n᥀︙للخروج ارسل ↫ ( الغاء ) \n ✓'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(Engineer.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(Engineer.."Abs:Send:Gp"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
List = DevAbs:smembers(Engineer..'Abs:Groups') 
if msg.content_.text_ then
for k,v in pairs(List) do 
AbsText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
...
DevAbs:setex(Engineer.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙ارسل الرساله الان لتوجيها \n᥀︙للخروج ارسل ↫ ( الغاء ) \n ✓'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(Engineer.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(Engineer.."Abs:Send:FwdGp"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false  
end 
local List = DevAbs:smembers(Engineer..'Abs:Groups')   
for k,v in pairs(List) do  
...
DevAbs:setex(Engineer.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙ارسل الرساله الان لتوجيها \n᥀︙للخروج ارسل ↫ ( الغاء ) \n ✓'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(Engineer.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == 'الغاء' then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(Engineer.."Abs:Send:FwdPv"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false  
end 
local List = DevAbs:smembers(Engineer..'Abs:Users')   
for k,v in pairs(List) do  
tdcli_function({ID="ForwardMessages", chat_id_ = v, from_chat_id_ = msg.chat_id_, message_ids_ = {[0] = msg.id_}, disable_notification_ = 0, from_background_ = 1},function(a,t) end,nil) 
end   
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم اذاعة رسالتك بالتوجيه \n᥀︙‏الى ↫ ❨ "..#List.." ❩ مشترك \n ✓", 1, 'md')
...
DevAbs:setex(Engineer.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_, 600, true) 
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙ارسل لي سواء ↫ ⤈ \n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n᥀︙للخروج ارسل ↫ ( الغاء ) \n ✓'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
return false
end 
if DevAbs:get(Engineer.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_) then 
if text == "الغاء" then   
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء امر الاذاعه بنجاح", 1, 'md')
DevAbs:del(Engineer.."Abs:Send:Gp:Pin"..msg.chat_id_..":" .. msg.sender_user_id_) 
return false
end 
local List = DevAbs:smembers(Engineer.."Abs:Groups") 
if msg.content_.text_ then
...
AbsText = "الرساله"
send(v, 0,"["..msg.content_.text_.."]") 
DevAbs:set(Engineer..'Abs:PinnedMsgs'..v,msg.content_.text_) 
end
elseif msg.content_.photo_ then
if msg.content_.photo_.sizes_[0] then
photo = msg.content_.photo_.sizes_[0].photo_.persistent_id_
...
AbsText = "الصوره"
sendPhoto(v, 0, 0, 1, nil, photo,(msg.content_.caption_ or ''))
DevAbs:set(Engineer..'Abs:PinnedMsgs'..v,photo) 
end 
elseif msg.content_.animation_ then
for k,v in pairs(List) do 
AbsText = "المتحركه"
sendDocument(v, 0, 0, 1,nil, msg.content_.animation_.animation_.persistent_id_,(msg.content_.caption_ or ''))    
DevAbs:set(Engineer..'Abs:PinnedMsgs'..v,msg.content_.animation_.animation_.persistent_id_)
end 
elseif msg.content_.video_ then
for k,v in pairs(List) do 
AbsText = "الفيديو"
sendVideo(v, 0, 0, 1, nil, msg.content_.video_.video_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(Engineer..'Abs:PinnedMsgs'..v,msg.content_.video_.video_.persistent_id_)
end 
elseif msg.content_.voice_ then
for k,v in pairs(List) do 
AbsText = "البصمه"
sendVoice(v, 0, 0, 1, nil, msg.content_.voice_.voice_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(Engineer..'Abs:PinnedMsgs'..v,msg.content_.voice_.voice_.persistent_id_)
end 
elseif msg.content_.audio_ then
for k,v in pairs(List) do 
AbsText = "الصوت"
sendAudio(v, 0, 0, 1, nil, msg.content_.audio_.audio_.persistent_id_,(msg.content_.caption_ or '')) 
DevAbs:set(Engineer..'Abs:PinnedMsgs'..v,msg.content_.audio_.audio_.persistent_id_)
end 
elseif msg.content_.document_ then
for k,v in pairs(List) do 
AbsText = "الملف"
sendDocument(v, 0, 0, 1,nil, msg.content_.document_.document_.persistent_id_,(msg.content_.caption_ or ''))    
DevAbs:set(Engineer..'Abs:PinnedMsgs'..v,msg.content_.document_.document_.persistent_id_)
end 
elseif msg.content_.sticker_ then
for k,v in pairs(List) do 
AbsText = "الملصق"
sendSticker(v, 0, 0, 1, nil, msg.content_.sticker_.sticker_.persistent_id_)   
DevAbs:set(Engineer..'Abs:PinnedMsgs'..v,msg.content_.sticker_.sticker_.persistent_id_) 
end 
end
...
local List = DevAbs:smembers(Engineer..'Abs:Manager:GpRedod'..msg.chat_id_)
if #List == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙لا توجد ردود متعدده مضافه" ,  1, "md")
return false
end
DevAbs:set(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedRedod')
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل كلمة الرد اولا" ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local DelGpRedRedod = DevAbs:get(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
...
if not DevAbs:sismember(Engineer..'Abs:Manager:GpRedod'..msg.chat_id_,text) then
...
DevAbs:set(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedRedods')
DevAbs:set(Engineer..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_,text)
return false
end end
...
DevAbs:set(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'DelGpRedod')
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل الكلمه لحذفها" ,  1, "md")
return false
end
if text == 'اضف رد متعدد' and Manager(msg) and ChCheck(msg) then
DevAbs:set(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'SetGpRedod')
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل الكلمه الان" ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local SetGpRedod = DevAbs:get(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_)
...
DevAbs:set(Engineer..'Abs:Add:GpRedod'..msg.sender_user_id_..msg.chat_id_,'SaveGpRedod')
DevAbs:set(Engineer..'Abs:Add:GpTexts'..msg.sender_user_id_..msg.chat_id_,text)
DevAbs:sadd(Engineer..'Abs:Manager:GpRedod'..msg.chat_id_,text)
return false
end end
--     Source Engineer     --
if text == 'حذف رد' and Manager(msg) and ChCheck(msg) or text == 'مسح رد' and  Manager(msg) and ChCheck(msg) then
local List = DevAbs:smembers(Engineer..'Abs:Manager:GpRed'..msg.chat_id_)
if #List == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙لا توجد ردود مضافه" ,  1, "md")
return false
end
DevAbs:set(Engineer..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'DelGpRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل الكلمه لحذفها " ,  1, "md")
return false
end
if text == 'اضف رد' and Manager(msg) and ChCheck(msg) then
DevAbs:set(Engineer..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'SetGpRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل الكلمه الان " ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local SetGpRed = DevAbs:get(Engineer..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_)
...
DevAbs:set(Engineer..'Abs:Add:GpRed'..msg.sender_user_id_..msg.chat_id_,'SaveGpRed')
DevAbs:set(Engineer..'Abs:Add:GpText'..msg.sender_user_id_..msg.chat_id_,text)
DevAbs:sadd(Engineer..'Abs:Manager:GpRed'..msg.chat_id_,text)
DevAbs:set(Engineer..'DelManagerRep'..msg.chat_id_,text)
return false
end end
--     Source Engineer     --
...
local List = DevAbs:smembers(Engineer.."Abs:Sudo:AllRed")
if #List == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙لا توجد ردود مضافه" ,  1, "md")
return false
end
DevAbs:set(Engineer.."Abs:Add:AllRed"..msg.sender_user_id_,'DelAllRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل الكلمه لحذفها " ,  1, "md")
return false
end
...
DevAbs:set(Engineer.."Abs:Add:AllRed"..msg.sender_user_id_,'SetAllRed')
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙حسنا ارسل الكلمه الان " ,  1, "md")
return false
end
if text and text:match("^(.*)$") then
local SetAllRed = DevAbs:get(Engineer.."Abs:Add:AllRed"..msg.sender_user_id_)
...
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙ارسل لي الرد سواء كان ↫ ⤈\n❨ ملف • ملصق • متحركه • صوره\n • فيديو • بصمه • صوت • رساله ❩\n᥀︙يمكنك اضافة الى النص ↫ ⤈\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n `#username` ↬ معرف المستخدم\n `#msgs` ↬ عدد الرسائل\n `#name` ↬ اسم المستخدم\n `#id` ↬ ايدي المستخدم\n `#stast` ↬ رتبة المستخدم\n `#edit` ↬ عدد السحكات\n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n᥀︙للخروج ارسل ↫ ( الغاء )\n ✓" ,  1, "md")
DevAbs:set(Engineer.."Abs:Add:AllRed"..msg.sender_user_id_,'SaveAllRed')
DevAbs:set(Engineer.."Abs:Add:AllText"..msg.sender_user_id_, text)
DevAbs:sadd(Engineer.."Abs:Sudo:AllRed",text)
DevAbs:set(Engineer.."DelSudoRep",text)
return false 
end end
--     Source Engineer     --
if text == "الساعه" then
...
MsgRep = MsgRep..k..'~ (`'..v..'`) • {*العدد ↫ '..#DevAbs:smembers(Engineer..'Abs:Text:GpTexts'..v..msg.chat_id_)..'*}\n' 
end
if #redod == 0 then
...
for k,v in pairs(redod) do
DevAbs:del(Engineer..'Abs:Text:GpTexts'..v..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Manager:GpRedod'..msg.chat_id_)
end
...
MsgRep = '᥀︙ردود المدير ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n'
for k,v in pairs(redod) do
if DevAbs:get(Engineer.."Abs:Gif:GpRed"..v..msg.chat_id_) then
dp = 'متحركه 🎭'
elseif DevAbs:get(Engineer.."Abs:Voice:GpRed"..v..msg.chat_id_) then
dp = 'بصمه 🎙'
elseif DevAbs:get(Engineer.."Abs:Stecker:GpRed"..v..msg.chat_id_) then
dp = 'ملصق 🃏'
elseif DevAbs:get(Engineer.."Abs:Text:GpRed"..v..msg.chat_id_) then
dp = 'رساله ✉'
elseif DevAbs:get(Engineer.."Abs:Photo:GpRed"..v..msg.chat_id_) then
dp = 'صوره 🎇'
elseif DevAbs:get(Engineer.."Abs:Video:GpRed"..v..msg.chat_id_) then
dp = 'فيديو 📽'
elseif DevAbs:get(Engineer.."Abs:File:GpRed"..v..msg.chat_id_) then
dp = 'ملف 📁'
elseif DevAbs:get(Engineer.."Abs:Audio:GpRed"..v..msg.chat_id_) then
dp = 'اغنيه 🎶'
end
...
if Manager(msg) then
if text and (text =='حذف الردود' or text == 'مسح الردود' or text == 'حذف ردود المدير' or text == 'مسح ردود المدير') and not DevAbs:get(Engineer..'Abs:Lock:Rd'..msg.chat_id_) and ChCheck(msg) then
local redod = DevAbs:smembers(Engineer..'Abs:Manager:GpRed'..msg.chat_id_)
if #redod == 0 then
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙لا توجد ردود مضافه" ,  1, "md")
else
for k,v in pairs(redod) do
DevAbs:del(Engineer..'Abs:Gif:GpRed'..v..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Voice:GpRed'..v..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Audio:GpRed'..v..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Photo:GpRed'..v..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Stecker:GpRed'..v..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Video:GpRed'..v..msg.chat_id_)
DevAbs:del(Engineer..'Abs:File:GpRed'..v..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Text:GpRed'..v..msg.chat_id_)
DevAbs:del(Engineer..'Abs:Manager:GpRed'..msg.chat_id_)
end
...
local redod = DevAbs:smembers(Engineer.."Abs:Sudo:AllRed")
MsgRep = '᥀︙ردود المطور ↫ ⤈ \n┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉ ┉\n'
for k,v in pairs(redod) do
if DevAbs:get(Engineer.."Abs:Gif:AllRed"..v) then
dp = 'متحركه 🎭'
elseif DevAbs:get(Engineer.."Abs:Voice:AllRed"..v) then
dp = 'بصمه 🎙'
elseif DevAbs:get(Engineer.."Abs:Stecker:AllRed"..v) then
dp = 'ملصق 🃏'
elseif DevAbs:get(Engineer.."Abs:Text:AllRed"..v) then
dp = 'رساله ✉'
elseif DevAbs:get(Engineer.."Abs:Photo:AllRed"..v) then
dp = 'صوره 🎇'
elseif DevAbs:get(Engineer.."Abs:Video:AllRed"..v) then
dp = 'فيديو 📽'
elseif DevAbs:get(Engineer.."Abs:File:AllRed"..v) then
dp = 'ملف 📁'
elseif DevAbs:get(Engineer.."Abs:Audio:AllRed"..v) then
dp = 'اغنيه 🎶'
end
MsgRep = MsgRep..k..'~ (`'..v..'`) ↫ {*'..dp..'*}\n' 
end
...
for k,v in pairs(redod) do
DevAbs:del(Engineer.."Abs:Add:AllRed"..v)
DevAbs:del(Engineer.."Abs:Gif:AllRed"..v)
DevAbs:del(Engineer.."Abs:Voice:AllRed"..v)
DevAbs:del(Engineer.."Abs:Audio:AllRed"..v)
DevAbs:del(Engineer.."Abs:Photo:AllRed"..v)
DevAbs:del(Engineer.."Abs:Stecker:AllRed"..v)
DevAbs:del(Engineer.."Abs:Video:AllRed"..v)
DevAbs:del(Engineer.."Abs:File:AllRed"..v)
DevAbs:del(Engineer.."Abs:Text:AllRed"..v)
DevAbs:del(Engineer.."Abs:Sudo:AllRed")
end
...
DevAbs:set(Engineer..'Abs:NameBot'..msg.sender_user_id_, 'msg')
return false 
end
end
...
DevAbs:del(Engineer..'Abs:NameBot')
local EngineerSource = '᥀︙اهلا عزيزي ↫ '..AbsRank(msg)..' \n᥀︙تم حذف اسم البوت'
Absmoned(msg.chat_id_, msg.sender_user_id_, msg.id_, EngineerSource, 14, string.len(msg.sender_user_id_))
end end 
--     Source Engineer     --
...
DevAbs:set(Engineer..'Abs:Help0'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local EngineerSource =  DevAbs:get(Engineer..'Abs:Help0'..msg.sender_user_id_)
if EngineerSource == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Engineer..'Abs:Help0'..msg.sender_user_id_)
DevAbs:set(Engineer..'Abs:Help', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(Engineer..'Abs:Help01'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local EngineerSource =  DevAbs:get(Engineer..'Abs:Help01'..msg.sender_user_id_)
if EngineerSource == 'msg' then 
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Engineer..'Abs:Help01'..msg.sender_user_id_)
DevAbs:set(Engineer..'Abs:Help1', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(Engineer..'Abs:Help21'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local EngineerSource =  DevAbs:get(Engineer..'Abs:Help21'..msg.sender_user_id_)
if EngineerSource == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Engineer..'Abs:Help21'..msg.sender_user_id_)
DevAbs:set(Engineer..'Abs:Help2', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(Engineer..'Abs:Help31'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local EngineerSource =  DevAbs:get(Engineer..'Abs:Help31'..msg.sender_user_id_)
if EngineerSource == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Engineer..'Abs:Help31'..msg.sender_user_id_)
DevAbs:set(Engineer..'Abs:Help3', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(Engineer..'Abs:Help41'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local EngineerSource =  DevAbs:get(Engineer..'Abs:Help41'..msg.sender_user_id_)
if EngineerSource == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Engineer..'Abs:Help41'..msg.sender_user_id_)
DevAbs:set(Engineer..'Abs:Help4', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده" ,  1, "md")
return false end
end
...
DevAbs:set(Engineer..'Abs:Help51'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local EngineerSource =  DevAbs:get(Engineer..'Abs:Help51'..msg.sender_user_id_)
if EngineerSource == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Engineer..'Abs:Help51'..msg.sender_user_id_)
DevAbs:set(Engineer..'Abs:Help5', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده " ,  1, "md")
return false end
end
...
DevAbs:set(Engineer..'Abs:Help61'..msg.sender_user_id_, 'msg')
return false end
if text and text:match("^(.*)$") then
local EngineerSource =  DevAbs:get(Engineer..'Abs:Help61'..msg.sender_user_id_)
if EngineerSource == 'msg' then
Dev_Abs(msg.chat_id_, msg.id_, 1, text , 1, 'md')
DevAbs:del(Engineer..'Abs:Help61'..msg.sender_user_id_)
DevAbs:set(Engineer..'Abs:Help6', text)
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ الكليشه الجديده" ,  1, "md")
return false end
end
...
local Users = DevAbs:smembers(Engineer.."User_Bot")
...
local id = Users[i]
if id:match("^(%d+)") then
DevAbs:sadd(Engineer..'Abs:Users',Users[i]) 
end
end
for i = 1, #Sudos do
DevAbs:sadd(Engineer..'Abs:SudoBot:',Sudos[i]) 
end
for i = 1, #Groups do
DevAbs:sadd(Engineer..'Abs:Groups',Groups[i]) 
if DevAbs:get(Engineer.."Private:Group:Link"..Groups[i]) then
DevAbs:set(Engineer.."Abs:Groups:Links"..Groups[i],DevAbs:get(Engineer.."Private:Group:Link"..Groups[i]))
end
if DevAbs:get(Engineer.."Get:Welcome:Group"..Groups[i]) then
DevAbs:set(Engineer..'Abs:Groups:Welcomes'..Groups[i],DevAbs:get(Engineer.."Get:Welcome:Group"..Groups[i]))
end
local list2 = DevAbs:smembers(Engineer..'Constructor'..Groups[i])
for k,v in pairs(list2) do
DevAbs:sadd(Engineer.."Abs:Constructor:"..Groups[i], v)
end
local list3 = DevAbs:smembers(Engineer..'BasicConstructor'..Groups[i])
for k,v in pairs(list3) do
DevAbs:sadd(Engineer.."Abs:BasicConstructor:"..Groups[i], v)
end
local list4 = DevAbs:smembers(Engineer..'Manager'..Groups[i])
for k,v in pairs(list4) do
DevAbs:sadd(Engineer.."Abs:Managers:"..Groups[i], v)
end
local list5 = DevAbs:smembers(Engineer..'Mod:User'..Groups[i])
for k,v in pairs(list5) do
DevAbs:sadd(Engineer.."Abs:Admins:"..Groups[i], v)
end
local list6 = DevAbs:smembers(Engineer..'Special:User'..Groups[i])
for k,v in pairs(list6) do
DevAbs:sadd(Engineer.."Abs:VipMem:"..Groups[i], v)
end
DevAbs:set(Engineer.."Abs:Lock:Bots"..Groups[i],"del") DevAbs:hset(Engineer.."Abs:Spam:Group:User"..Groups[i] ,"Spam:User","keed") 
...
for i,Lock in pairs(LockList) do
DevAbs:set(Engineer..Lock..Groups[i],true)
end
end
...
local Get_Files, res = https.request("https://raw.githubusercontent.com/EngineerSource/EngineerFiles/main/getfile.json")
if res == 200 then
...
local GetJson, Res = https.request("https://raw.githubusercontent.com/EngineerSource/EngineerFiles/main/EngineerFiles/"..FileName)
if Res == 200 then
...
DevAbs:del(Engineer..'Abs:Text:BotWelcome')
DevAbs:del(Engineer..'Abs:Photo:BotWelcome')
return false
end 
...
DevAbs:del(Engineer..'Abs:Lock:BotWelcome')
return false
end 
...
DevAbs:set(Engineer..'Abs:Lock:BotWelcome',true)
return false
end 
...
DevAbs:del(Engineer..'Abs:Text:BotWelcome')
DevAbs:del(Engineer..'Abs:Photo:BotWelcome')
DevAbs:set(Engineer.."Abs:Set:BotWelcome"..msg.sender_user_id_,"Text") 
return false
end 
if text and DevAbs:get(Engineer.."Abs:Set:BotWelcome"..msg.sender_user_id_) == 'Text' then 
if text and text:match("^الغاء$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء الامر", 1, "md") 
DevAbs:del(Engineer.."Abs:Set:BotWelcome"..msg.sender_user_id_)   
return false
end 
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم حفظ النص ارسل لي صورة الترحيب\n᥀︙ارسل ↫ الغاء لحفظ النص فقط", 1, 'md')   
DevAbs:set(Engineer.."Abs:Text:BotWelcome",text) 
DevAbs:set(Engineer.."Abs:Set:BotWelcome"..msg.sender_user_id_,"Photo") 
return false 
end 
if DevAbs:get(Engineer.."Abs:Set:BotWelcome"..msg.sender_user_id_) == 'Photo' then 
if text and text:match("^الغاء$") then 
...
DevAbs:set(Engineer.."Abs:Photo:BotWelcome",msg.content_.photo_.sizes_[1].photo_.persistent_id_)
...
DevAbs:setex(Engineer.."Abs:DevText"..msg.chat_id_..":" .. msg.sender_user_id_, 300, true)
end end
...
DevAbs:del(Engineer.."DevText")
end end
--     Source Engineer     --
if DevAbs:get(Engineer.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") then 
Dev_Abs(msg.chat_id_, msg.id_, 1, "᥀︙تم الغاء الامر", 1, "md") 
DevAbs:del(Engineer.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
return false  end 
DevAbs:del(Engineer.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_)  
local texxt = string.match(text, "(.*)") 
DevAbs:set(Engineer..'Abs:ChText',texxt)
...
DevAbs:setex(Engineer.."textch:user"..msg.chat_id_.."" .. msg.sender_user_id_, 300, true)   
...
DevAbs:del(Engineer..'Abs:ChText')
...
DevAbs:incr(Engineer..'Abs:EditMsg'..result.chat_id_..result.sender_user_id_)
local text = result.content_.text_ or result.content_.caption_
local Text = result.content_.text_
if DevAbs:get(Engineer..'Abs:Lock:EditMsgs'..msg.chat_id_) and not Text and not SecondSudo(result) then
...
local GetMsgPin = DevAbs:get(Engineer..'Abs:PinnedMsgs'..msg.chat_id_)
if GetMsgPin ~= nil then
if text == GetMsgPin then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) if dp.ID == 'Ok' then;DevAbs:del(Engineer..'Abs:PinnedMsgs'..msg.chat_id_);end;end,nil)   
elseif (msg.content_.sticker_) then 
if GetMsgPin == msg.content_.sticker_.sticker_.persistent_id_ then
tdcli_function ({ID = "PinChannelMessage",channel_id_ = msg.chat_id_:gsub('-100',''),message_id_ = msg.id_,disable_notification_ = 0},function(arg,dp) DevAbs:del(Engineer..'Abs:PinnedMsgs'..msg.chat_id_) end,nil)   
end
end
if (msg.content_.animation_) then 
...
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusMember" then
tdcli_function({ID = "ChangeChatMemberStatus",chat_id_=v,user_id_=Engineer,status_={ID = "ChatMemberStatusLeft"},},function(e,g) end, nil) 
DevAbs:srem(Engineer..'Abs:Groups',v)  
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
DevAbs:srem(Engineer..'Abs:Groups',v)  
end
...
DevAbs:sadd(Engineer..'Abs:Groups',v)  
end end,nil) end
end
--     Source Engineer     --
end 
